...
export interface HeartbeatOptions {
  intervalMs?: number;
  onTick?: (lastStatus: any) => void;
  endpoint?: string;
  enabled?: boolean;
  maxConsecutiveFailures?: number;
  onFailureThreshold?: (failures: number, lastUpdated: Date | null) => void;
}

export interface HeartbeatResult<T = any> {
  status: T | null;
 ...
 */
export function useHeartbeat<T = any>(options: HeartbeatOptions = {}): HeartbeatResult<T> {
  const {
    intervalMs = 15000,
    onTick,
  ...
  const lastEmissionRef = useRef<number>(0); // For debouncing event emissions
