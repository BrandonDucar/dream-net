...
export function SafeFarcasterSolanaProvider({ endpoint, children }: SafeFarcasterSolanaProviderProps): React.ReactNode {
  ...
  // Cast to any to bypass JSX type mismatch
  const InternalProvider = FarcasterSolanaProvider as any;

  return (
    <SolanaProviderContext.Provider value={{ hasSolanaProvider }}>
      {hasSolanaProvider ? (
        <InternalProvider endpoint={endpoint}>
          {children}
        </InternalProvider>
      ) : (
        <>{children}</>
