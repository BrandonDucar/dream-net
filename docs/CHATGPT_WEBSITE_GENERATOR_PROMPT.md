# DreamNet Token Website - ChatGPT Website Generator Prompt

## How to Use This

1. Open ChatGPT
2. Select the **"Website Generator"** GPT (or search for it)
3. Copy the prompt below
4. Paste it into ChatGPT
5. Let it generate the site
6. Click "Publish" → Get your `*.pixl.site` URL
7. Buy domain and point CNAME to PIXL URL
...
```
Create a production-ready, multi-page website for DreamNet Token (DREAM) on Base mainnet.
...
- **Loading states**: Elegant loading animations (spinner, skeleton screens)
...
1. **Review the generated site** in ChatGPT's preview
2. **Iterate until it's perfect** - Don't settle! Request adjustments like:
   - "Make the hero section more impactful - larger text, more spacing"
   - "Add more subtle neon glow to buttons and cards"
   - "Increase the glassmorphism effect on cards"
   - "Make the dark background richer/deeper"
   - "Add subtle animations to the hero section"
   - "Improve mobile spacing - more breathing room"
   - "Make buttons more prominent with better hover effects"
   - "Enhance typography - bolder headings, better contrast"
   - "Add subtle gradient overlays to sections"
   - "Make the overall design feel more premium and polished"
3. **Test on mobile** - Make sure it looks stunning on phone too
4. **Click "Publish"** → Get your `*.pixl.site` URL
5. **Test the URL** - it should work immediately
6. **Buy domain** (e.g., `dreamnettoken.xyz`)
7. **Point DNS**:
   ```
   Type: CNAME
   Name: @
   ...
   ```
8. **Wait for SSL** (automatic, usually 1-30 minutes)

**Remember**: This is your official front door. Take time to make it beautiful. Iterate until you're proud to show it off.
...
## Why This Works Better Than Vercel

✅ **No build failures** - PIXL just hosts static files
✅ **Instant SSL** - Automatic, no configuration
✅ **Simple DNS** - Just CNAME, that's it
✅ **No deployment drama** - Publish = live
...
1. Update BaseScan token profile with your new domain
2. Link mini apps from DreamHub page
3. Add real DEX links when pool exists
...
- **Iterate in ChatGPT**: Ask for changes until you like it
- **Test mobile**: Make sure it looks good on phone
- **Check all links**: Verify BaseScan links work
- **Copy button**: Make sure contract address copy works
- **Save the prompt**: Keep this prompt for future updates
