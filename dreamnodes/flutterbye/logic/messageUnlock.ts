...
export function checkMessageUnlock(
  userTrustScore: number,
  messageAgentSource?: string
): MessageUnlockStatus {
...
export function getUnlockProgress(userTrustScore: number): {
  percentage: number;
...
export function analyzeMessagePattern(message: string): {
  supportive: boolean;
  emoji: boolean;
  trustImpact: number;
} {
  const supportiveKeywords = ['got this', 'you can', 'believe', 'strength', 'support'];
  const supportive = supportiveKeywords.some(keyword => 
  ...
  let trustImpact = 0;
  if (supportive) trustImpact += 1;
  if (emoji) trustImpact += 0.5;
  if (message.length > 10 && message.length < 100) trustImpact += 0.5;
