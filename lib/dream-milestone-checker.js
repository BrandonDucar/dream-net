...
export async function checkDreamMilestones(dream, userPhone) {
    ...
    if (dream.collaborators?.length >= 3 && !dream.flags?.calledOnTeam) {
        await sendDreamCallSMS(userPhone, dream.id, "3+ collaborators joined. Your dream is building a movement.");
