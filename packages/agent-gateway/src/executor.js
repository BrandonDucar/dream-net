...
import { evaluateConduit, recordConduitFailure } from "@dreamnet/dreamnet-control-core/conduitGovernor";
...
export async function executeTool(toolId, payload, req) {
    ...
    try {
        ...
        if (!result.ok && conduitDecision.conduitId) {
            recordConduitFailure(conduitDecision.conduitId, result.error || "UNKNOWN_ERROR", isTimeout);
        }
