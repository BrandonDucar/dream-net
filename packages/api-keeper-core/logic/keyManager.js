 ...
 */
export function registerKey(providerId, key, secret, options) {
    const apiKey = {
        id: nextKeyId(),
        providerId,
 ...
 */
export function updateKeyStatus(keyId, status, reason) {
 ...
 * Record API usage
 */
export function recordUsage(keyId, cost = 0) {
    const key = APIStore.getKey(keyId);
    if (!key)
        return false;
    const now = Date.now();
 ...
 */
export function getBestKey(providerId) {
