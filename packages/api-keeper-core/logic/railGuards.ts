 ...
 */
export function checkRailGuards(request: APIRequest): {
  allowed: boolean;
  reason?: string;
} {
 ...
 */
export function ensureDefaultRailGuards(): APIRailGuard[] {
  const existing = APIStore.listRailGuards();
  if (existing.length > 0) return existing;

  const now = Date.now();
  const guards: APIRailGuard[] = [
    {
      id: nextGuardId(),
 ...
 */
export function createRailGuard(
  name: string,
  type: APIRailGuard["type"],
  limit: number,
  action: APIRailGuard["action"]
): APIRailGuard {
  const guard: APIRailGuard = {
    id: nextGuardId(),
    name,
    type,
    limit,
