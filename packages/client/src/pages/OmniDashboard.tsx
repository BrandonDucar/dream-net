    ...
    useEffect(() => {
        ...
        socketInstance.on('swarm_log', (log: SwarmLog) => {
            ...
            if (log.source === 'CORE' && log.message.includes('Homeostasis')) {
                // Mocking an event update for visual continuity
                setEvent(prev => prev || {
                    type: 'PULSE',
                    timestamp: Date.now(),
                    organ: 'CORE',
        ...
        <div className="min-h-screen bg-[#050505] text-white font-mono p-6 overflow-hidden relative selection:bg-cyan-500 selection:text-black">
            ...
            <main className="grid grid-cols-12 gap-6 relative z-10 h-[calc(100vh-140px)]">
                ...
                <div className="col-span-4 space-y-6 flex flex-col h-full overflow-hidden">
                    <section className="flex-1 bg-black/40 border border-white/5 p-4 rounded-xl backdrop-blur-md overflow-hidden flex flex-col">
                        ...
                        <div
                            ref={scrollRef}
                            className="space-y-2 overflow-y-auto flex-1 pr-2 custom-scrollbar text-[10px]"
                        >
                ...
                {/* CENTER: THE THINKING STREAM (The O1-style visualization) */}
                    ...
                    <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-4">
                        {logs.filter(l => l.source === 'GENOME_PILOT' || l.source === 'CORE').slice(0, 10).map((thought, i) => (
                            <motion.div
                                ...
                                className="bg-white/[0.03] border border-white/5 p-4 rounded-2xl relative"
                            >
                                ...
                                <div className="mt-2 flex gap-2">
                                    <div className="px-2 py-0.5 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-[8px] text-cyan-400">
                                        LATENCY: 42ms
                                    </div>
                                    <div className="px-2 py-0.5 rounded-full bg-purple-500/10 border border-purple-500/20 text-[8px] text-purple-400">
                                        CONFIDENCE: 0.99
                                    </div>
                                </div>
                            </motion.div>
                        ...
                        {logs.length === 0 && (
                            <div className="flex flex-col items-center justify-center h-full text-gray-500 opacity-20">
                ...
                <div className="col-span-3 space-y-4 overflow-y-auto pr-2 custom-scrollbar">
                    <NodeCard
                        ...
                        color="text-blue-400"
                        status="CORE_LINK_STABLE"
                        details="Uplink Port 5000 Active"
                    />
                    <NodeCard
                        ...
                        color="text-pink-500"
                        status="RESONATING"
                        details="AegisScion grounding complete"
                    />
                    <NodeCard
                        title="METABOLIC SIFTER"
                        icon={<Trash2 className="w-4 h-4" />}
                        color="text-yellow-500"
                        status="WEAPONIZED"
                        details="V2 Auto-Patcher Online"
                    />
                    <NodeCard
                        title="ORACLE"
                        icon={<Lock className="w-4 h-4" />}
                        color="text-green-500"
                        status="SYNTHTIC_GROUNDED"
                        details="Verified Truth Settlement"
                    />
            ...
            <footer className="mt-6 flex justify-between items-center text-[10px] text-gray-600 border-t border-white/5 pt-4 relative z-10">
                <div className="flex gap-4">
                    <span className="flex items-center gap-1"><Wifi className="w-3 h-3 text-green-500" /> SOCKET_UPTIME: {(process.uptime() / 60).toFixed(2)}m</span>
                    <span>IDENTITY: DREAM_PRIME</span>
                </div>
                <div className="animate-pulse flex items-center gap-2">
                    <Terminal className="w-3 h-3" />
                    SYSTEM_READY_FOR_JOIN_EXECUTION
                </div>
            </footer>
        </div>
    );
