...
export function computeRecommendations(
  ...
  const scored: OfferRecommendation[] = offers
 ...
 */
export function syncOffersWithReputation(ctx: ShopContext) {
  ...
  offers.forEach((offer) => {
    if (!offer.ownerIdentityId) return;

    const rep = scores.find(
      (s: any) => `${s.entityType}:${s.entityId}` === offer.ownerIdentityId
    );
