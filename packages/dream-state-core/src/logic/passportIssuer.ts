import { DreamPassport, DreamPassportTier } from '../types.js';
 ...
 */
export function getPassport(identityId: string): DreamPassport | undefined {
 ...
 */
export function upgradePassport(identityId: string, newTier: DreamPassportTier): DreamPassport | undefined {
  return CitizenshipStore.upgradePassport(identityId, newTier);
}

// Helper: determine if new tier is higher than existing
function shouldUpgrade(existing: DreamPassportTier, newTier: DreamPassportTier): boolean {
  const tierOrder: DreamPassportTier[] = ["visitor", "dreamer", "citizen", "operator", "architect", "founder"];
