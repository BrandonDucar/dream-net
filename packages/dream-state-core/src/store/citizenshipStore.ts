...
export const CitizenshipStore = {
  ...
  status(): DreamStateStatus {
    ...
    return {
      ...
      citizenCount: allPassports.length, // Mapping passport count to citizen count for now
      ...
      sampleCitizens: allPassports.slice(0, 20).map(p => ({
        identityId: p.identityId,
        passportNumber: p.passportNumber,
        citizenshipTier: p.tier,
        joinedAt: p.issuedAt,
        contributions: 0
      })),
...
function getVoteWeightForTier(tier: DreamPassportTier): number {
  switch (tier) {
    ...
    case "citizen":
      return 2;
    case "ambassador":
      return 2.5; // Ambassadors get a boost
    case "operator":
      return 3;
    case "architect":
      return 5;
