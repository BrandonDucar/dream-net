 ...
 */
export async function requestMint(userId, amount) {
 ...
 */
export async function requestBurn(userId, amount) {
    await recordDreamTokenEvent({
        id: randomUUID(),
        type: "burn",
        userId,
 ...
 */
export async function syncOnchainForUser(userId) {
 ...
 */
export async function moveToClaimable(userId, amount) {
    const account = await getOnchainAccount(userId);
    const internalBalance = await getUserBalance(userId);
