 ...
 */
import { getClusterConfig } from "./clusters";
import { TIERS } from "./tierConfig";
import { checkAndConsume } from "./rateLimiter";
import { emitControlCoreEvent } from "./eventFabric";
import { evaluatePolicy } from "./policyEngine";
import { NERVE_BUS } from "../nerve/src/bus.js";
import { createRequestDecisionEvent } from "../nerve/src/factory.js";
 ...
 */
export function setGlobalKillSwitch(enabled) {
 ...
 */
export function isGlobalKillSwitchEnabled() {
...
export function controlCoreMiddleware(req, res, next) {
    ...
    if (callerIdentity && !isGodVault) {
        ...
        const rateLimitResult = checkAndConsume(clusterId, callerIdentity.tierId, effectiveRateLimit);
        ...
        const rateLimitResult = checkAndConsume(clusterId, callerIdentity?.tierId || "GOD_MODE", effectiveRateLimit);
        remainingRateLimit = rateLimitResult.remaining;
        if (process.env.DEBUG_RATE_LIMIT === "true") {
 ...
 */
export function withCluster(clusterId) {
 ...
 */
export function withGovernance(options) {
    return (req, _res, next) => {
