...
import { sendSMS } from './twilioSender.js';
import { formatEventToSMS } from './messageFormatter.js';
...
const messageHistory: Array<{ timestamp: number; eventType: string }> = [];

/**
 * Check if we should send this message (rate limiting + priority filtering)
 */
function shouldSendMessage(event: OperationalEvent): boolean {
  ...
  // Filter by priority
  const sendableSeverities: OperationalEvent["severity"][] = ["critical", "high"];
 ...
 */
export async function routeEventToSMS(event: OperationalEvent): Promise<{ sent: boolean; reason?: string }> {
  if (!shouldSendMessage(event)) {
    return { sent: false, reason: "Filtered by router" };
  }

  try {
    const messageBody = formatEventToSMS(event);
