...
export class DetectorGenerator {
    ...
    detectors = new Map();
    MAX_DETECTORS = 1000;
    MIN_FITNESS = 0.3; // Remove detectors below this fitness
     ...
     */
    generateRandomDetector(categories, generation = 1) {
        const pattern = {};
     ...
     */
    async testDetectorAgainstSelf(detector) {
        for (const [metricName, range] of Object.entries(detector.pattern)) {
            // Find baseline for this metric
            const categories = ["service_health", "deployment", "resource_usage", "integration"];
            for (const category of categories) {
     ...
     */
    async generateDetectors(count = 100, categories = ["service_health", "deployment", "resource_usage", "integration"]) {
        const newDetectors = [];
        let attempts = 0;
     ...
     */
    testValue(detector, metricName, value) {
     ...
     */
    updateDetectorFitness(detectorId, isTruePositive, isFalsePositive, isFalseNegative, isTrueNegative) {
        ...
        if (isFalseNegative)
            detector.falseNegatives++;
        if (isTrueNegative)
            detector.trueNegatives++;
     ...
     */
    detectorToAnomalyPattern(detector) {
        const signature = {};
