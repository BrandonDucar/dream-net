...
export class NonSelfDetector {
     ...
     */
    async detectAnomaly(category, metricName, currentValue, threshold = 2.0 // z-score threshold (default: 2 standard deviations)
    ) {
        ...
        const percentileConfidence = (currentValue < baseline.statistics.percentile25 ||
     ...
     */
    async detectAnomalies(metrics, threshold = 2.0) {
        const detections = [];
        // Detect individual anomalies
        for (const metric of metrics) {
            const detection = await this.detectAnomaly(metric.category, metric.metricName, metric.value, threshold);
            if (detection) {
     ...
     */
    async detectRateOfChangeAnomaly(category, metricName, currentValue, previousValue, timeDelta // milliseconds between measurements
    ) {
        const baseline = await this.selfBaseline.getBaseline(category, metricName);
        if (!baseline) {
            return null;
