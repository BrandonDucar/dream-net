...
import StagedResponseSystem, { type StagedResponse, type ResponseStage } from "./stagedResponse.js";
...
export class ResponseRouter {
  private stagedResponseSystem: StagedResponseSystem;
  private historicalResponses: HistoricalResponse[] = [];
  ...
  async routeAnomaly(
    ...
    const response = this.stagedResponseSystem.createResponse(
      anomaly.id,
      anomaly.severity,
      anomaly.confidence,
      blastRadius,
   ...
   * Get historical success rate for similar anomalies
   */
  private getHistoricalSuccessRate(anomaly: AnomalyDetection): number | undefined {
  ...
  getRoutingStats(): {
    ...
    const byStage: Record<ResponseStage, number> = { 0: 0, 1: 0, 2: 0, 3: 0 };
