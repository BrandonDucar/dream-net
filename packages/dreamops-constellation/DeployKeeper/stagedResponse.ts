...
export interface StagedResponse {
  ...
  blastRadius: number; // How many services affected (0-1)
  startedAt: string;
  completedAt?: string;
...
export interface ResponseAction {
  ...
  target: string; // Service, agent, or component
  description: string;
  executed: boolean;
  executedAt?: string;
...
export class StagedResponseSystem {
  private responses: Map<string, StagedResponse> = new Map();
   ...
   */
  async executeResponse(responseId: string): Promise<StagedResponse> {
    ...
    if (failedActions.length > 0 && response.stage < 3) {
      response.escalatedTo = (response.stage + 1) as ResponseStage;
    }
   ...
   */
  async escalateResponse(responseId: string): Promise<StagedResponse> {
    const response = this.responses.get(responseId);
    if (!response) {
      throw new Error(`Response ${responseId} not found`);
    }
    ...
    const nextStage = (response.stage + 1) as ResponseStage;
    const newActions = this.generateActionsForStage(
      nextStage,
      response.metadata?.affectedServices || [],
      response.severity
    );
