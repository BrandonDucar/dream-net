...
export class IntegratedConstellationOrchestrator extends ConstellationOrchestrator {
    ...
    async handleDevBrief(brief) {
        ...
        try {
            ...
            await this.telegram.sendOpsCard(`Dev Brief: ${brief.title}`, "✅", `Created GitHub issue #${issue.number}`, issue.url);
        }
        catch (error) {
    ...
    async handleDeployOrder(order) {
        ...
        try {
            ...
            deployment = await this.deployKeeper.deploy(order.service, order.environment, order.branch);
            // Store in memory
            ...
            // Send Telegram notification
            await this.telegram.sendOpsCard(`Deployment: ${order.service}`, deployment.status === "success" ? "✅" : "⚠️", `Deploying to ${order.environment}`, result.url);
            ...
            // Send alert
            await this.telegram.sendIncident(order.service, "high", `Deployment failed: ${error.message}`);
        }
    }
    /**
    ...
    async handleContentBrief(brief) {
        ...
        try {
            ...
            const post = await this.socialWeaver.postNow(brief.description, brief.platforms, undefined, { source: "dreamops", briefId: brief.id });
            // Store in memory
            ...
            // Send Telegram notification
            await this.telegram.sendContentThread(brief.title, brief.description, brief.platforms, { source: "dreamops" });
        }
        catch (error) {
    ...
    async handleDeploymentStatus(status) {
        ...
        await this.telegram.sendOpsCard(`Deployment: ${status.service}`, emoji, `Status: ${status.status}`, status.url);
    }
    /**
     * Handle Starbridge deployment events
     */
    async handleStarbridgeDeploymentEvent(payload) {
     ...
     */
    async handleStarbridgeSocialEvent(payload) {
    ...
    async startAutonomousCycle() {
        ...
        for (const deployment of deployments) {
            if (deployment.url) {
                ...
                if (health.status !== "ok") {
                    await this.telegram.sendIncident(deployment.service, "medium", `Health check failed: ${health.status}`);
                }
            }
        }
