 ...
 */
function getFounderWalletAddresses(): string[] {
 ...
 */
const DREAMSTATE_BOOTSTRAP: DreamStateSnapshot = {
  version: 1,
 ...
 */
export const DREAMSTATE: DreamStateSnapshot = DREAMSTATE_BOOTSTRAP;
 ...
 */
export function getPassportByWallet(wallet: string): DreamPassport | undefined {
  const normalizedWallet = wallet.toLowerCase().trim();
  
  for (const passport of Object.values(DREAMSTATE.passports)) {
 ...
 * @returns Passport if found, undefined otherwise
 */
export function getPassportByCitizenId(citizenId: CitizenId): DreamPassport | undefined {
  return DREAMSTATE.passports[citizenId];
}
 ...
 */
export function getOfficesForCitizen(citizenId: CitizenId): Office[] {
  const passport = getPassportByCitizenId(citizenId);
  if (!passport) return [];
  
  return passport.officeIds
    .map((officeId) => DREAMSTATE.offices[officeId])
 ...
 */
export function getCabinetsForCitizen(citizenId: CitizenId): Cabinet[] {
  const passport = getPassportByCitizenId(citizenId);
  if (!passport) return [];
  
  return passport.cabinetIds
    .map((cabinetId) => DREAMSTATE.cabinets[cabinetId])
