...
function point(kind: FieldPointId["kind"], id: string): FieldPointId {
  return { kind, id };
}

export function updateFieldsFromReputation(ctx: FieldContext, now: number) {
  if (!ctx.reputationLattice?.status) return;
...
export function updateFieldsFromStarBridge(ctx: FieldContext, now: number) {
  if (!ctx.starBridge?.status) return;
...
export function updateFieldsFromQAL(ctx: FieldContext, now: number) {
  if (!ctx.quantumAnticipation?.status) return;
...
export function updateFieldsFromDreamCortex(ctx: FieldContext, now: number) {
  if (!ctx.dreamCortex?.listDreams || !ctx.dreamCortex?.status) return;
  ...
  dreams.forEach((d: any) => {
    ...
    if (d.status === "completed") base -= 0.3;
    if (d.status === "blocked" || d.status === "infected") base += 0.2;
...
export function updateFieldsFromWolfPackAndPSL(ctx: FieldContext, now: number) {
...
function mapRepTypeToFieldKind(entityType: string): FieldPointId["kind"] {
  switch (entityType) {
    case "dream":
      return "dream";
    case "agent":
...
function mapDecayTargetToKind(targetType: string): FieldPointId["kind"] {
  switch (targetType) {
    case "route":
    case "service":
      return "service";
