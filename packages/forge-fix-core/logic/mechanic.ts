...
export class ForgeMechanic {
    ...
    static async diagnose(error: ErrorEvent): Promise<PatchProposal> {
        ...
        // [DURABLE HANDOFF] Injecting workflow-authority context
        const context = {
            error: error,
            memoryRetrieval: await vectorStore.search(`fix error ${error.message}`, 3),
            environment: "Production/DreamNet",
            workflowStage: "Analysis", // Mission 95: Tracking state
