 ...
 */
export function watchEvent(event, source = "mesh") {
    const newAlerts = [];
    const now = Date.now();
    // Update last seen
 ...
 */
export async function prowlTerritories() {
 ...
 */
export function getAlerts(authorized = false) {
    if (!authorized) {
 ...
 * Increase Base fame
 */
export function increaseFame(amount = 1) {
