...
import { getMemoryRecord, listMemoryRecords, upsertMemoryRecord } from './dnaStore.js';
...
export async function ensureRecord(entityType, entityId) {
    ...
    try {
        await vectorStore.addMemory(`New ${entityType} entity created: ${entityId} at Hive Location q=${nextCoordinate.q},r=${nextCoordinate.r}`, { type: "entity_creation", entityType, entityId, coordinates: nextCoordinate });
    }
    catch (err) {
...
export async function updateTraitsFromEvent(event) {
    ...
    try {
        ...
        await vectorStore.addMemory(`Event: ${summary}`, { type: "event", eventType: event.eventType, severity: event.severity });
    }
    catch (err) {
        console.warn("Vector embedding failed (non-critical):", err);
    }
...
export async function logSystemMemory(action, metadata = {}) {
    // [SELF-REFLECTION]
    // Allows the System (Antigravity) to log its own evolution
    try {
        await vectorStore.addMemory(`SYSTEM ACTION: ${action}`, { type: "system_evolution", source: "Antigravity", ...metadata });
    }
    catch (err) {
