...
export async function updateTraitsFromEvent(event: EventModel): Promise<void> {
  ...
  // [MEMVERSE INTEGRATION] Structured Cognition Injection
  try {
    const memverse = new (await import('./adapters/MemVerseAdapter.js')).MemVerseAdapter();
    await memverse.ingest({
      type: event.severity === 'critical' ? 'core' : 'episodic',
      content: event.payload?.summary || `Event: ${event.eventType}`,
      modality: 'text',
      metadata: {
        agentId: event.sourceId,
        eventType: event.eventType,
        confidence: 0.9, // Default confidence for system events
        context: event.payload
      }
    });

    // Periodic consolidation check (Simplified for prototype)
    if (Math.random() < 0.01) {
      await memverse.consolidate();
    }
  } catch (err) {
    console.warn("MemVerse ingestion failed (non-critical):", err);
  }
