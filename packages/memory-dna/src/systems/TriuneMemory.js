 ...
 */
export class LizardLayer {
    name = 'LIZARD';
    cache = new Map();
    async store(key, value, ttlSeconds = 300) {
 ...
 */
export class MammalLayer {
    name = 'MAMMAL';
    associativeNodes = new Map();
     ...
     */
    async recallRelational(keyContext) {
        return Array.from(this.associativeNodes.values())
            .filter(node => node.tags?.includes(keyContext) ||
            node.id === keyContext ||
    ...
    async searchByTag(tag) {
        return Array.from(this.associativeNodes.values()).filter(node => node.tags?.includes(tag) || node.reflection?.includes(tag));
    }
...
export class CosmicLayer {
    ...
    log = [];
    currentHMAC = 'genesis';
    SECRET = process.env.COSMIC_SECRET || 'DREAMNET_ARCHITECTURAL_SOUL';
    constructor() {
        this.load();
    }
    async store(key, value) {
...
export class TriuneMemory {
    lizard = new LizardLayer();
    mammal = new MammalLayer();
    cosmic = new CosmicLayer();
    async initialize() {
     ...
     */
    async remember(key, value, context) {
        switch (context) {
            case 'SURVIVAL':
