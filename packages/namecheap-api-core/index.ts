...
export class NamecheapApiClient {
  ...
  private parseXmlResponse(xml: string): any {
    try {
      ...
      if (apiResponse['@_Status'] === 'ERROR' || apiResponse.Status === 'ERROR') {
        ...
        throw new Error(
          `Namecheap API Error: ${errorMessages.join('; ') || 'Unknown error'}`
        );
      }
   ...
   */
  async getDnsRecords(domain: string): Promise<NamecheapDnsRecord[]> {
    try {
   ...
   */
  async setDnsRecords(domain: string, records: Array<{
    hostname: string;
   ...
   * Get domain info
   */
  async getDomainInfo(domain: string): Promise<NamecheapDomain | null> {
