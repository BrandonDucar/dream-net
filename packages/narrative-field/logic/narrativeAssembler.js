...
export function assembleNarratives(ctx) {
    const now = Date.now();
    const entries = [];
    ...
    if (ctx.dreamCortex?.status) {
        ...
        directives.forEach((d) => {
            const severity = d.intent === "unblock" || d.intent === "deprecate"
                ? "warning"
                : d.intent === "stabilize"
                    ? "notice"
                    : "info";
            entries.push({
                ...
                summary: `Dream '${d.dreamId}' received directive '${d.intent}' with confidence ${(d.confidence ?? 0).toFixed(2)}.`,
                severity,
                domain: "dream",
...
//# sourceMappingURL=narrativeAssembler.js.map