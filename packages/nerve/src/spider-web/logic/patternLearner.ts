import { ThreadPattern, FlyPattern, SignalThread, Fly } from '../types.js';
 ...
 */
export function learnThreadPatterns(): ThreadPattern[] {
  const completedThreads = SpiderStore.listThreadsByStatus("completed");
  const newPatterns: ThreadPattern[] = [];

  // Group threads by kind
  const threadsByKind = new Map<string, SignalThread[]>();
  ...
  for (const [kind, threads] of threadsByKind.entries()) {
    if (threads.length < 3) continue; // Need at least 3 examples
    ...
    const existingPattern = Array.from(SpiderStore.listThreadPatterns()).find(
      (p) => p.threadKind === kind
    );
    ...
    if (existingPattern) {
      ...
      // Create new pattern
      const pattern: ThreadPattern = {
        id: nextPatternId(),
        pattern: `Thread pattern for ${kind}`,
        threadKind: kind as SignalThread["kind"],
        successRate,
        avgExecutionTime,
        usageCount: threads.length,
        discoveredAt: Date.now(),
      };
      SpiderStore.addThreadPattern(pattern);
 ...
 */
export function learnFlyPatterns(): FlyPattern[] {
  const processedFlies = SpiderStore.listFlies().filter((f) => f.processed && f.threadId);
  const newPatterns: FlyPattern[] = [];

  // Group flies by type and source
  const fliesByTypeSource = new Map<string, Fly[]>();
  ...
  for (const [key, flies] of fliesByTypeSource.entries()) {
    if (flies.length < 3) continue; // Need at least 3 examples
    ...
    const threadKinds = new Map<string, number>();
    for (const fly of flies) {
      if (fly.threadId) {
        const thread = SpiderStore.getThread(fly.threadId);
        if (thread) {
    ...
    // Check if pattern already exists
    const existingPattern = Array.from(SpiderStore.listFlyPatterns()).find(
      (p) => p.flyType === flyType && p.source === source
    );
    ...
    if (existingPattern) {
      ...
      // Create new pattern
      const pattern: FlyPattern = {
        id: nextPatternId(),
        flyType: flyType as Fly["type"],
        source,
        commonPayloadKeys,
        typicalThreadKind: typicalThreadKind as SignalThread["kind"],
        frequency: flies.length,
        discoveredAt: Date.now(),
      };
      SpiderStore.addFlyPattern(pattern);
      newPatterns.push(pattern);
