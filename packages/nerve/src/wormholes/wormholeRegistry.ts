...
export function createWormhole(data: Partial<WormholeModel> & { name: string; from: WormholeModel["from"]; to: WormholeModel["to"] }): WormholeModel {
  ...
  const wormhole: WormholeModel = {
    ...
    name: data.name,
    label: data.name, // Default label to name
    description: data.description,
    fiber: (data as any).fiber || "magrail-0", // Default fiber if not provided
    direction: (data as any).direction || "out", // Default direction
    from: data.from,
    to: data.to,
