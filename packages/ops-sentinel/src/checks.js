 ...
 */
export function validateVercelConfig(contract, repoRoot = process.cwd()) {
    const errors = [];
    const warnings = [];
    ...
    try {
        ...
        if (vercelJson.rootDirectory !== expected.rootDirectory) {
            errors.push(`vercel.json rootDirectory must be "${expected.rootDirectory}", got "${vercelJson.rootDirectory}"`);
        }
        // Check installCommand
        if (vercelJson.installCommand !== expected.installCommand) {
            warnings.push(`vercel.json installCommand should be "${expected.installCommand}", got "${vercelJson.installCommand}"`);
        }
        // Check buildCommand
        if (vercelJson.buildCommand !== expected.buildCommand) {
            warnings.push(`vercel.json buildCommand should be "${expected.buildCommand}", got "${vercelJson.buildCommand}"`);
        }
        // Check outputDirectory
        if (vercelJson.outputDirectory !== expected.outputDirectory) {
            errors.push(`vercel.json outputDirectory must be "${expected.outputDirectory}", got "${vercelJson.outputDirectory}"`);
        }
        // Check rewrites
        if (!vercelJson.rewrites || !Array.isArray(vercelJson.rewrites)) {
            ...
            const apiRewrite = vercelJson.rewrites.find((r) => r.source === '/api/:path*');
            if (!apiRewrite) {
 ...
 */
export function validatePackageScripts(contract, repoRoot = process.cwd()) {
    const errors = [];
    const warnings = [];
    ...
    if (existsSync(clientPackageJsonPath)) {
        try {
            const clientPackageJson = JSON.parse(readFileSync(clientPackageJsonPath, 'utf-8'));
 ...
 */
export function validateRepoStructure(repoRoot = process.cwd()) {
    const errors = [];
    const warnings = [];
 ...
 */
export function validateRepoSetup(contract, repoRoot = process.cwd()) {
    const allErrors = [];
    const allWarnings = [];
    // Run all validations
