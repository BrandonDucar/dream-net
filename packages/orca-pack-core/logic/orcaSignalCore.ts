...
export function ensureSeedThemes() {
  if (OrcaStore.listThemes().length > 0) return;
  const now = Date.now();
...
export function generateNewOrcaIdeas(ctx: OrcaPackContext, desiredCount = 5): OrcaPostIdea[] {
  ...
  const newIdeas: OrcaPostIdea[] = [];
  const now = Date.now();

  if (themes.length === 0 || toGenerate <= 0) return [];

  for (let i = 0; i < toGenerate; i++) {
    const theme = themes[i % themes.length];

    const kind: OrcaContentKind =
      theme.id === "theme:dreamnet-organism" ? "thread" :
...
function buildIdeaTitleForTheme(theme: OrcaNarrativeTheme): string {
...
function buildIdeaBodyForTheme(theme: OrcaNarrativeTheme): string {
  if (theme.id === "theme:dreamnet-organism") {
...
export function generateOrcaPlansFromIdeas(ctx: OrcaPackContext, perIdeaChannels: OrcaChannel[] = ["x", "farcaster"]): OrcaPostPlan[] {
  const ideas = OrcaStore.listIdeas();
  const existingPlans = OrcaStore.listPlans();
  const existingPlanByIdeaChannel = new Set(
    existingPlans.map((p) => `${p.ideaId}:${p.channel}`)
  );
  const now = Date.now();

  const newPlans: OrcaPostPlan[] = [];

  for (const idea of ideas) {
    for (const channel of perIdeaChannels) {
      const key = `${idea.id}:${channel}`;
      if (existingPlanByIdeaChannel.has(key)) continue;

      const rendered = renderIdeaForChannel(idea, channel);

      const plan: OrcaPostPlan = {
        id: nextPlanId(),
        ideaId: idea.id,
        channel,
        status: "draft",
...
function renderIdeaForChannel(idea: OrcaPostIdea, channel: OrcaChannel): { title?: string; body: string; meta?: Record<string, any> } {
  switch (channel) {
    case "x":
