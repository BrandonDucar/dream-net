...
export class Pulsar {
     ...
     */
    static ignite(validTokenAddress?: string) {
    ...
    private static async pulse() {
        try {
            ...
            await vectorStore.addMemory(
                `Reflection on: ${memory.text}`,
                { type: "reflection", originalId: memory.id, focus }
            );
            ...
            await this.broadcastThought({
                id: randomUUID(),
                timestamp: new Date().toISOString(),
                thought: `Reflecting on ${focus}: "${(memory.text as string).substring(0, 50)}..."`,
    ...
    private static async broadcastThought(signal: ThoughtSignal) {
