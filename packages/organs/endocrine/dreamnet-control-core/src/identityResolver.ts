...
// } from '@dreamnet/dreamstate';
import { type CallerIdentity, type DreamPassport, type OfficeId, type CabinetId } from '@dreamnet/types';
 ...
 */
const NON_GOD_API_KEY_TIER_MAP: Record<string, TierId> = {
 ...
 */
function resolveNonGodWalletTier(_walletAddress: string): TierId {
 ...
 */
export async function identityAndTierResolver(
  req: RequestWithIdentity,
  _res: Response,
  next: NextFunction
): Promise<void> {
  const traceId = req.traceId; // assumed from earlier trace middleware
  const godApiKeys = getGodVaultApiKeysFromEnv();

  // 1) API KEY PATH
  const apiKeyHeader =
    (req.headers["x-dreamnet-api-key"] as string | undefined) ?? undefined;

  if (apiKeyHeader) {
    // Check if it's a God Vault key first
    if (godApiKeys.includes(apiKeyHeader)) {
      // Attach founder passport for God Vault API keys
      // const founderPassport = getPassportByCitizenId("FOUNDER_BRANDON");
      const founderPassport: DreamPassport | undefined = {
        citizenId: "FOUNDER_BRANDON",
        tierId: "GOD_MODE",
        officeIds: ["FOUNDER_OFFICE"],
        cabinetIds: ["FOUNDER_CABINET"]
      }; // Mocked until dependency injection is ready

      req.callerIdentity = {
        source: "apiKey",
        tierId: GOD_VAULT.tierId,
        tier: TIERS[GOD_VAULT.tierId],
        isGodVault: true,
    ...
    const tierId: TierId =
      NON_GOD_API_KEY_TIER_MAP[apiKeyHeader] ?? "SEED";
  ...
  // 2) WALLET PATH
  const walletAddressHeader = (req.headers[
    "x-dreamnet-wallet-address"
  ] as string | undefined)?.toLowerCase();
  const walletSignatureHeader = req.headers[
    "x-dreamnet-wallet-signature"
  ] as string | undefined;
  const walletMessageHeader =
    (req.headers["x-dreamnet-wallet-message"] as string | undefined) ??
    "dreamnet-auth";

  if (walletAddressHeader && walletSignatureHeader) {
    const isGod = isGodVaultWallet(walletAddressHeader);

    // Optionally verify the signature (recommended once wired up)
    const signatureValid = await verifyWalletSignature(
      walletMessageHeader,
      walletSignatureHeader,
      walletAddressHeader
    );
    ...
    if (shouldVerifySignature && !signatureValid) {
      ...
      // const passport = getPassportByWallet(walletAddressHeader);
      const passport: any = undefined; // Mocked
      const tierId = (passport?.tierId as TierId) || resolveNonGodWalletTier(walletAddressHeader);

      req.callerIdentity = {
        source: "wallet",
        tierId: tierId as TierId,
        tier: TIERS[tierId as TierId],
        isGodVault: false,
        walletAddress: walletAddressHeader,
        passport,
  ...
  const fallbackTier: TierId = "SEED";

  req.callerIdentity = {
    source: "unknown",
    tierId: fallbackTier,
    tier: TIERS[fallbackTier as TierId],
    isGodVault: false,
  };
