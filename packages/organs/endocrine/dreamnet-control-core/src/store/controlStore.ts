...
class ControlStore {
  ...
  private rateLimits: Map<ClusterId, ClusterRateLimit> = new Map();
  private requestCounts: Map<ClusterId, Array<{ timestamp: number }>> = new Map();
  private circuitBreakers: Map<ClusterId, { tripped: boolean; tripCount: number; lastTripAt?: number; autoResetAfter?: number }> = new Map();

  // Tier-based rate limiting
  private tierRequestCounts: Map<TierId, Array<{ timestamp: number }>> = new Map();
  private tierConcurrencyCounts: Map<TierId, number> = new Map();
  ...
  private initializeDefaultRateLimits() {
    const defaultLimits: ClusterRateLimit[] = [
      {
        clusterId: "WOLF_PACK",
  ...
  getConfig(): ControlConfig {
    const clusterStates: Record<string, any> = {};
