...
export async function discoverFromVercel() {
    ...
    try {
        ...
        for (const project of projects) {
            try {
                const envResponse = await axios.get(`${VERCEL_API_BASE}/projects/${project.id}/env${teamId ? `?teamId=${teamId}` : ""}`, {
                    headers: { Authorization: `Bearer ${token}` },
                });
                ...
                for (const vercelEnv of envVars) {
                    ...
                    const category = vercelEnv.key.includes("API") && vercelEnv.key.includes("KEY") ? "api_keys" :
...
//# sourceMappingURL=vercelSync.js.map