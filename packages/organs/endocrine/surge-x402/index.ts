...
export interface SurgePurchase {
  budget: number;
  minQuotientScore: number;
  interactionType: 'likes' | 'recasts';
  castHash: string;
  buyerFid?: number;
}
...
const purchaseHistory: SurgeResult[] = [];
const activeCampaigns: Map<string, CampaignConfig> = new Map();
 ...
 */
export async function buyEngagement(
  castHash: string,
  ...
  try {
    ...
    await bridge.broadcast(
      `[SURGE] Buying ${type} for cast ${castHash.slice(0, 12)}... (budget: $${budget} USDC)`,
      purchase,
      'high',
    );
    ...
    await bridge.broadcast(
      `[SURGE] SUCCESS: ${result.engagementCount || '?'} ${type} purchased for $${result.usdcSpent} USDC`,
      result,
      'high',
    );
 ...
 */
export async function boostCast(
  castHash: string,
  likeBudget: number = MAX_BUDGET,
  recastBudget: number = MAX_BUDGET,
  minScore: number = MIN_SCORE,
): Promise<{ likes: SurgeResult; recasts: SurgeResult }> {
...
export function createCampaign(config: CampaignConfig): void {
  activeCampaigns.set(config.castHash, config);
  bridge.broadcast(
    `[SURGE] Campaign created for cast ${config.castHash.slice(0, 12)}...`,
    config,
    'low',
  );
}

export function getCampaign(castHash: string): CampaignConfig | undefined {
...
export function cancelCampaign(castHash: string): boolean {
  return activeCampaigns.delete(castHash);
}

// ─── History & Stats ──────────────────────────────────────────────────────

export function getPurchaseHistory(limit = 50): SurgeResult[] {
