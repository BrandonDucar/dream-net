    ...
    if (shield && shield.wormholeConnected) {
      ...
      switch (signalType) {
        ...
        case "frequency-sync":
          ...
          break;
        case "spike-command":
          // Cell receives spike command: Anchor to Orbital Status
          const { cortex: shieldCortex } = require('../../../../server/src/services/SensoryCortex.js');
          const orbSnapshot = shieldCortex.getLatestSnapshot();
          if (orbSnapshot && orbSnapshot.planetary?.satellite) {
            (shield as any).orbitalRef = orbSnapshot.planetary.satellite.active_satellites > 140 ? 'orbital-stable' : 'orbital-shifty';
            console.log(`[CellularShield] 🛰️ Signal anchored to Orbital State: ${(shield as any).orbitalRef}`);
          }
          break;
      }
