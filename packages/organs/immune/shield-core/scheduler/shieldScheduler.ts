import { ShieldContext, ShieldStatus } from '../types.js';
...
import { ensureDefaultModulators } from '../logic/shieldModulator.js';
import { ensureDefaultEmitters } from '../logic/shieldEmitter.js';
import { simulateThreatDetection, analyzeThreat } from '../logic/threatDetector.js';
import { fireSpikeAtThreat } from '../logic/offensiveSpike.js';
import { createCellularShield, propagateShieldViaWormhole, getCellularShieldStats } from '../logic/cellularShield.js';
import { learnFromThreats, predictThreatSeverity } from '../logic/shieldLearner.js';
import { initializeCrossChainShield, syncCrossChainShields, getCrossChainShieldStats } from '../logic/crossChainShield.js';
 ...
 */
export function runShieldCycle(ctx: ShieldContext): ShieldStatus {
  const now = Date.now();
  ...
  for (const threat of detectedThreats) {
    ...
    if (analysis.shouldBlock) {
      ...
      if (analysis.recommendedSpike) {
        const spike = fireSpikeAtThreat(
          threat,
          analysis.recommendedSpike as any
        );
        if (spike) {
  ...
  if (!ShieldStore.getLayer("omega-base" as any)) {
