...
// import { ReputationLattice } from '@dreamnet/reputation-lattice'; // Potential circular dep, use interface or decoupled event

interface WatchtowerConfig {
    fraudProofWindowMs: number; // e.g. 1000ms for testing, 7 days in prod
    sampleRate: number; // 0.0 - 1.0 (1.0 = Verify Everything)
}
...
export class ShieldWatchtower {
    ...
    constructor(config: WatchtowerConfig = { fraudProofWindowMs: 5000, sampleRate: 0.1 }) {
        this.config = config;
    }
    ...
    public start() {
        ...
        NERVE_BUS.subscribe('shield-watchtower', (event: NerveEvent) => {
    ...
    private async handleOptimisticRollup(event: NerveEvent) {
    ...
    private async verifyProof(payload: any): Promise<boolean> {
        ...
        if (payload?.context?.riskScore > 90) return false;

        return true;
    }

    private async slashAgent(agentId: string, tierId: string, reason: string) {
