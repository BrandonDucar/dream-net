 ...
 */
function extractKeywords(title: string, description: string): SEOKeyword[] {
 ...
 */
function optimizeTitle(title: string, platform: SEOPlatform, keywords: SEOKeyword[]): string {
  const maxLength: Record<SEOPlatform, number> = {
    web: 60,
    twitter: 280,
    farcaster: 320,
    instagram: 150,
 ...
 */
function optimizeDescription(description: string, platform: SEOPlatform, keywords: SEOKeyword[]): string {
  const maxLength: Record<SEOPlatform, number> = {
    web: 160,
    twitter: 280,
    farcaster: 320,
...
function calculateSEOScore(
  ...
  if (title.length > 0 && title.length <= 60) score += 30;
 ...
 * Generate meta tags
 */
function generateMetaTags(keywords: SEOKeyword[], platform: SEOPlatform): string[] {
  const tags: string[] = [];
...
function determineIntent(keyword: string): SEOKeyword["intent"] {
  ...
  if (commercial.some((term) => lower.includes(term))) return "commercial";
  if (transactional.some((term) => lower.includes(term))) return "transactional";
  if (navigational.some((term) => lower.includes(term))) return "navigational";
