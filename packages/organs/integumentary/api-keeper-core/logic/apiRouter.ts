 ...
 */
export function routeRequest(request: APIRequest): APIRoutingDecision | null {
  ...
  if (request.requiredFeatures && request.requiredFeatures.length > 0) {
    candidates = candidates.filter((p) =>
      request.requiredFeatures!.every((f) => p.features.includes(f))
    );
  }
  ...
  const decision: APIRoutingDecision = {
