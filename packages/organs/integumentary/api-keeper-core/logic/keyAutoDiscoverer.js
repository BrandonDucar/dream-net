 ...
 */
export function autoDiscoverKeysFromEnv() {
    const discovered = [];
        ...
        for (const config of configs) {
            ...
            if (!alreadyRegistered) {
                ...
                if (config.providerId === "twilio" && envKey === "TWILIO_ACCOUNT_SID") {
                    ...
                    if (authToken) {
                        const key = registerKey(config.providerId, value, authToken, {
                    ...
                    const key = registerKey(config.providerId, value, undefined, {
                        name: `Auto-discovered ${config.name}`,
                        tags: ["auto-discovered", "env", "zero-touch"],
                    });
                    discovered.push(key);
        ...
        for (const { pattern, providerHint } of GENERIC_PATTERNS) {
            ...
            if (match) {
                ...
                if (provider) {
                    ...
                    if (!alreadyRegistered) {
                        const key = registerKey(provider.id, value, undefined, {
                    ...
                    const key = registerKey(hint, value, undefined, {
 ...
 */
export function autoDiscoverAllKeys() {
    const discovered = [];
        ...
        if (!process.env[key]) {
            ...
            if (provider) {
                ...
                if (!existing.some((k) => k.key === value)) {
                    const registered = registerKey(provider.id, value, undefined, {
 ...
 */
export function continuousAutoDiscovery() {
    // Run full discovery
    return autoDiscoverAllKeys();
}
//# sourceMappingURL=keyAutoDiscoverer.js.map