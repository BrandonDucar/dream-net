import type { CivicPanelContext, CivicPanelStatus } from '../types.js';
import { CommandStore } from '../store/commandStore.js';
import { buildDashboardSnapshot } from '../logic/statusAggregator.js';
import { processCommands } from '../logic/commandHandler.js';

export async function runCivicPanelCycle(ctx: CivicPanelContext): Promise<CivicPanelStatus> {
  const now = Date.now();
