...
import { EvolutionEngine, DREAMKEEPER_CORE, DreamDefenseNet, SurgeonAgent } from '@/lib/mock-dreamnet';
                ...
                {surgeonTasks.length > 0 ? (
                  surgeonTasks.map((task, i) => (
                    <div key={i} className="p-3 bg-gray-800 rounded border border-green-400/20">
                      <div className="flex items-center justify-between mb-1">
                        <Badge className={`text-xs ${task.resolved ? 'bg-green-500 text-white' : 'bg-yellow-500 text-black'
                          }`}>
                ...
                {threats.length > 0 ? (
                  threats.map((threat, i) => (
                    <div key={i} className="p-3 bg-gray-800 rounded border border-red-400/20">
                      <div className="flex items-center justify-between mb-1">
                        <Badge className={`text-xs ${threat.severity === 'critical' ? 'bg-red-500 text-white' :
