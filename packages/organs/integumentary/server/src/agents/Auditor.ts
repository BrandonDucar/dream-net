...
export class AuditorAgent implements Agent<AuditorRequest, AuditorResponse> {
    ...
    public capabilities = ['security', 'build', 'inventory', 'auditing'];
    public circadianPulse = 3600000; // 1 hour heartbeat
    ...
    public async pulse(ctx: AgentInvocationContext): Promise<AgentResult> {
        console.log(`[Auditor] 🕒 Circadian Pulse Triggered: Performing System-Wide Health Scan...`);
        try {
            const result = await this.run({ checkBuild: true }, ctx);

            // Check for Work Problems
            if (result.buildStability && result.buildStability.score < 80) {
                agentBus.broadcast('WORK_PROBLEM', `Build stability is critical (${result.buildStability.score}%). Recovery required.`, {
                    agent: this.id,
                    stability: result.buildStability
                });
            }

            // Identify Exploits/Abundance (e.g., high balance detected)
            const totalValue = result.inventory.reduce((sum, item) => sum + item.usdValue, 0);
            if (totalValue > 5000) {
                agentBus.broadcast('OPPORTUNITY_DETECTED', `Meta-Abundance detected: $${totalValue.toFixed(2)}. Consider strategic swaps.`, {
                    agent: this.id,
                    totalValue
                });
            }
            ...
            await BlackboardScheduler.postTask(this.id, {
                ...
                action: `Build: ${result.buildStability?.score || 0}% | Inventory: ${result.inventory.length} assets.`
            }, "System Auditor");

            // Post to Social Hub
            await BlackboardScheduler.postChatter(this.id, `Audit complete. Build is ${result.buildStability?.score || 0}% stable. No major traumatic regressions found.`);
