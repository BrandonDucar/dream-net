...
class DreamNetEmail {
  ...
  constructor() {
    // Default to smtp if WOLFMAIL credentials exist, else console
    const hasSmtp = !!(process.env.WOLFMAIL_SMTP_HOST && process.env.WOLFMAIL_SMTP_PASS);

    this.config = {
      provider: (process.env.EMAIL_PROVIDER as EmailProvider) || (hasSmtp ? "smtp" : "console"),
      from: process.env.WOLFMAIL_FROM_EMAIL || process.env.DREAMNET_EMAIL || "dreamnetgmo@gmail.com",
      fromName: process.env.WOLFMAIL_FROM_NAME || "DreamNet Wolf Pack",
      replyTo: process.env.DREAMNET_REPLY_TO || "dreamnetgmo@gmail.com",
      smtpHost: process.env.WOLFMAIL_SMTP_HOST || "smtp.gmail.com",
      smtpPort: parseInt(process.env.WOLFMAIL_SMTP_PORT || "465"),
      smtpUser: process.env.WOLFMAIL_SMTP_USER || process.env.WOLFMAIL_FROM_EMAIL,
      smtpPass: process.env.WOLFMAIL_SMTP_PASS,
      smtpSecure: process.env.WOLFMAIL_SMTP_SECURE === "true" || process.env.WOLFMAIL_SMTP_PORT === "465",
  ...
  private async sendViaResend(message: EmailMessage): Promise<void> {
    ...
    try {
      ...
      console.log(`[DreamNet Email] Sent successfully via Resend to ${message.to}`);
    } catch (error) {
      console.error(`[DreamNet Email] Resend error:`, error);
      throw error;
    }
  }
  ...
  private async sendViaSMTP(message: EmailMessage): Promise<void> {
    try {
      ...
      console.log(`[DreamNet Email] Sent successfully via SMTP to ${message.to}`);
    } catch (error) {
      console.error(`[DreamNet Email] SMTP error:`, error);
      throw error;
    }
  }
