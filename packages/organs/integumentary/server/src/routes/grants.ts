  ...
  isDatabaseAvailable().then(available => {
    if (!available) {
      if (process.env.NODE_ENV === 'production') {
        console.error('❌ [GRANTS] CRITICAL: Database unavailable in production. Wolf Pack pipeline offline.');
      } else {
        console.log('🏆 [GRANTS] Database unavailable, mock routes will be registered separately for dev');
      }
    } else {
