 ...
 */
export function trainOnLeadScoring(leads: FundingLead[]): LearnedPattern[] {
  ...
  const highPriorityContacted = highPriority.filter(
    (l) => l.stage !== "new" && l.stage !== "dead"
  );
  ...
  // Store patterns
  patterns.forEach((p) => AnalystStore.upsertPattern(p));
 ...
 */
export function trainOnEmailEffectiveness(
