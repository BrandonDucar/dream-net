...
export async function ingestChemical(agent, chemicalId) {
    ...
    // Log Metabolic Event
    await vectorStore.addMemory(`METABOLIC EVENT: Agent ${agent.entityId} ingested ${chemical.name}. Effects: ${affectedTraits.join(", ")}`, { type: "ingestion", agentId: agent.entityId, chemicalId, effects: chemical.effects });
    return agent;
}
export function listAvailableChemicals() {
