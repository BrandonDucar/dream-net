...
import type { UserBalances, RewardEvent, RewardType } from './types.js';

export function calculateDailyReward(streakDays: number = 0): { dream: number; sheep: number } {
...
export function calculateWeeklyReward(): { dream: number; sheep: number } {
  return {
...
function canClaimLogin(lastLoginAt?: string): boolean {
...
function canClaimDaily(lastDailyClaimAt?: string): boolean {
...
function canClaimWeekly(lastWeeklyClaimAt?: string): boolean {
...
export async function grantReward(
  ...
  let deltaDream = 0;
  let deltaSheep = 0;
  let updatedBalance: UserBalances = { ...balance };
