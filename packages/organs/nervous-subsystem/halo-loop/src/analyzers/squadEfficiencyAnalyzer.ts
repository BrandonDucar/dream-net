...
async function fetchSquadMetrics(): Promise<SquadMetrics[]> {
  try {
    const response = await fetch(
      `${process.env.SQUAD_API_BASE ?? "http://127.0.0.1:5000"}/api/squad/metrics`,
    );
    if (response.ok) {
      const data = (await response.json()) as { squads: SquadMetrics[] };
...
export async function squadEfficiencyAnalyzer(): Promise<AnalyzerResult> {
  ...
  squads.forEach((squad) => {
    const failureRate =
      squad.completedTasks + squad.failedTasks === 0
        ? 0
