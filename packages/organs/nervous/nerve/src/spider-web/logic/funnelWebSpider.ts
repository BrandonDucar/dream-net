import { SpiderWebContext, SignalThread, WebSensor, Fly, FlyType, FlyPriority } from '../types.js';
import { SpiderStore } from '../store/spiderStore.js';
import { catchFly, createFly } from './flyCatcher.js';
import { checkTwilioAPI, checkTelegramAPI, checkTwitterAPI } from './apiIntegrations.js';
 ...
 */
async function checkSensor(sensor: WebSensor, ctx: SpiderWebContext): Promise<Fly[]> {
  const flies: Fly[] = [];
  const now = Date.now();
  ...
  switch (sensor.type) {
    case "twilio":
      ...
      if (sensor.config.accountSid && sensor.config.authToken) {
        try {
          const apiFlies = await checkTwilioAPI(
            sensor.config.accountSid,
            sensor.config.authToken,
            sensor.config.phoneNumber
          );
          flies.push(...apiFlies);
        } catch (err) {
      ...
      if (sensor.config.simulateFlies && flies.length === 0) {
        const fly = createFly(
          "message",
          "twilio",
          { message: "Simulated Twilio SMS", from: "+1234567890" },
          "medium",
          true
        );
        flies.push(fly);
    ...
    case "telegram":
      ...
      if (sensor.config.botToken) {
        try {
          const apiFlies = await checkTelegramAPI(
            sensor.config.botToken,
            sensor.config.chatId
          );
          flies.push(...apiFlies);
        } catch (err) {
      ...
      if (sensor.config.simulateFlies && flies.length === 0) {
        const fly = createFly(
          "message",
          "telegram",
          { message: "Simulated Telegram message", chatId: "12345" },
          "medium",
          true
        );
        flies.push(fly);
    ...
    case "twitter":
      ...
      if (sensor.config.bearerToken) {
        try {
          const apiFlies = await checkTwitterAPI(
            sensor.config.bearerToken,
      ...
      if (sensor.config.simulateFlies && flies.length === 0) {
        const fly = createFly(
          "mention",
          "twitter",
          { tweet: "Simulated Twitter mention", username: "@dreamnet" },
          "low",
          false
        );
        flies.push(fly);
    ...
    case "webhook":
      ...
      if (sensor.config.simulateFlies) {
        const fly = createFly(
          "webhook",
          "webhook",
          { type: "funding", data: { leadId: "lead:test" } },
          "high",
          true
        );
        flies.push(fly);
    ...
    case "blockchain":
      ...
      if (sensor.config.simulateFlies) {
        const fly = createFly(
          "transaction",
          "blockchain",
          { txHash: "0x123...", value: "1000000000000000000" },
          "high",
          true
        );
        flies.push(fly);
    ...
    case "email":
      ...
      if (sensor.config.simulateFlies) {
        const fly = createFly(
          "message",
          "email",
          { subject: "Simulated email", from: "test@example.com" },
          "medium",
          true
        );
        flies.push(fly);
 ...
 */
export async function runFunnelWebSpider(ctx: SpiderWebContext): Promise<SignalThread[]> {
  const newThreads: SignalThread[] = [];
 ...
 */
export function ensureDefaultSensors(): WebSensor[] {
 ...
 */
function updateSensorsFromEnv(): void {
