...
export class EventGraphQLService {
     ...
     * Directive 002: Secured by Agent Identity.
     */
    private async reverseSiphon(
        agentId: string,
        challenge: string,
        signature: string,
        query: string,
        ...
        console.log(`[🛰️ EventGraphQL] 🌀 Initiating Secured Reverse Siphon: "${query}" for ${agentId}`);

        // 1. Pull semantic matches from ChronoLoom (Secured)
        const searchResults = await chronoLoom.searchTimeline(
            agentId,
            challenge,
            signature,
            query,
    ...
    private async getRecentEvents(limit: number): Promise<any[]> {
        // Retrieve real-time event buffer from ChronoLoom
        return chronoLoom.getTimelineSlice(limit);
    }
