 ...
 */
export function getRuntimeStatusSnapshot(): RuntimeStatusViewModel {
      ...
      if (runtimeBridge?.RuntimeBridgeCore && dreamNetOS?.DreamNetOSCore) {
        ...
        if (osSnapshot) {
          return {
            ...
            subsystems: (osSnapshot.subsystems ?? []).map((sub: any) => ({
              name: sub.name ?? "Unknown",
              status: (sub.status ?? "unknown") as "ok" | "warn" | "error" | "unknown",
              details: sub.details,
            })),
          };
        }
 ...
 */
function getMockedRuntimeStatus(): RuntimeStatusViewModel {
  const now = Date.now();
