...
export class GeopoliticalSpike implements SensorySpike {
    ...
    async fetch(): Promise<SpikeResult> {
        try {
            ...
            const gdeltP = axios.get(
                'https://api.gdeltproject.org/api/v2/doc/doc?query=(conflict%20OR%20unrest%20OR%20election)&mode=artlist&format=json&maxresults=5'
            ).catch(() => ({ data: { articles: [] } }));

            // 2. NewsAPI Risk Sweep
            const newsP = this.newsApiKey ? axios.get(
                `https://newsapi.org/v2/everything?q=(war%20OR%20crisis%20OR%20embargo)&sortBy=publishedAt&pageSize=5&apiKey=${this.newsApiKey}`
            ).catch(() => ({ data: { articles: [] } })) : Promise.resolve({ data: { articles: [] } });

            const [gdelt, news] = await Promise.all([gdeltP, newsP]);

            const riskSignals = [
                ...(gdelt.data.articles || []).map((a: any) => a.title),
                ...(news.data.articles || []).map((a: any) => a.title)
            ];
