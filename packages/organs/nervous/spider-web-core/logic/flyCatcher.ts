...
export function shouldFlyStick(fly: Fly): boolean {
  ...
  if (fly.priority === "critical") return true;
  
  // High priority flies usually stick
  if (fly.priority === "high") return fly.sticky || Math.random() > 0.2;
  
  // Medium priority flies stick if marked sticky
  if (fly.priority === "medium") return fly.sticky || Math.random() > 0.5;
  
  // Low priority flies rarely stick unless explicitly sticky
  if (fly.priority === "low") return fly.sticky;
