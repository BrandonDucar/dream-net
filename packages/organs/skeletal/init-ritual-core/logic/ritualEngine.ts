...
export function ensureDefaultTemplateSeeded() {
  ...
  const defaultTemplate: InitFlowTemplate = {
 ...
 */
export function advanceIdentityInit(
  ctx: InitRitualContext,
  identityId: string,
  templateId: string = "default-user-init"
  ...
  // Determine new stage
  let newStage: InitStage = state.stage;
  if (state.stage === "not-started") newStage = "welcome";
  else if (state.stage === "welcome") newStage = "choose-dream-seed";
  else if (state.stage === "choose-dream-seed") newStage = "link-ritual";
  else if (state.stage === "link-ritual") newStage = "first-action";
  else if (state.stage === "first-action") newStage = "completed";
 ...
 */
export function completeInitStepForIdentity(
  identityId: string,
  stepId: string
): IdentityInitState {
