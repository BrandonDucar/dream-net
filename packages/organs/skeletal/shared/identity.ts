...
/**
 * Standardized DreamNet response wrapper.
 * Used for all API responses to ensure consistency.
 */
export interface DreamNetResponse<T = any> {
  success: boolean;
  data: T;
  error?: string;
  message?: string;
...
export function getIdentityAgents(identityId: string): string[] {
  ...
  const edges = IdentityGrid.listEdges().filter(
    (e: { fromId: string; toId: string; linkType: string }) =>
      e.fromId === identityId &&
      (e.linkType === "controls" || e.linkType === "owns") &&
      e.toId.startsWith("agent:")
  );

  return edges.map((e: { toId: string }) => e.toId);
}
