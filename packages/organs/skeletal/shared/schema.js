...
export const dreams = pgTable("dreams", {
    ...
    contributors: jsonb("contributors").$type().default(sql `'[]'::jsonb`),
    ...
    bountyToken: text("bounty_token").$type(), // Bounty token type
    bountyAmount: text("bounty_amount").default('1000000000000000000'), // 1.0 in token units
    dreamCloud: text("dream_cloud").$type(),
    // Evolution tracking
    evolutionType: text("evolution_type").$type(),
    ...
    // Blessings system
    blessings: jsonb("blessings").$type().default(sql `'[]'::jsonb`),
});
// Cocoons table
...
export const superSpineAgents = pgTable("super_spine_agents", {
    ...
    health: jsonb("health").$type().default(sql `'{"uptime": 100, "successRate": 100, "avgResponseTime": 0, "errorCount": 0}'::jsonb`),
...
export const superSpineSubscriptions = pgTable("super_spine_subscriptions", {
    ...
    price: jsonb("price").$type().notNull(),
});
...
export const mediaAssets = pgTable("media_assets", {
    ...
    rights: text("rights").$type().default("unknown"),
    rating: text("rating").$type().default("C"),
    collections: text("collections").array().default(sql `'{}'::text[]`),
    usage: jsonb("usage").$type().default(sql `'{"posts": 0, "downloads": 0, "last_used_at": null}'::jsonb`),
...
export const postQueue = pgTable("post_queue", {
    ...
    status: text("status").$type().default("scheduled"),
...
export const chakraSensors = pgTable("chakra_sensors", {
    id: varchar("id").primaryKey().default(sql `gen_random_uuid()`),
    userId: text("user_id").notNull(),
    chakraType: text("chakra_type").$type().notNull(),
...
export const metabolicScraps = pgTable("metabolic_scraps", {
    ...
    wasteType: text("waste_type").$type().notNull(),
