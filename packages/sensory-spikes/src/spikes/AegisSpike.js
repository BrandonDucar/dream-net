...
export class AegisSpike {
    name = 'AegisSpike';
    type = 'news'; // Categorized as Surveillance/News
    ...
    async fetch() {
        try {
            ...
            const response = await axios.get(`https://opensky-network.org/api/states/all?lamin=${this.lamin}&lomin=${this.lomin}&lamax=${this.lamax}&lomax=${this.lomax}`);
            const aircraft = response.data.states || [];
            const droneActivity = aircraft.map((a) => ({
                ids: a[0],
                callsign: a[1].trim(),
                country: a[2],
                lat: a[6],
                ...
                velocity: a[9],
                altitude: a[7]
            })).filter((a) => a.altitude < 2000); // Low altitude filter for "Drone-like" activity
            return {
...
//# sourceMappingURL=AegisSpike.js.map