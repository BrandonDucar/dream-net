...
export class MemPalaceSpike {
    ...
    setupListeners() {
        ...
        NERVE_BUS.subscribe('dreamnet.agents.arya.logs.*', async (event) => {
            await this.recordGrudge(event);
        });
        this.isConnected = true;
    }
    async recordGrudge(event) {
        try {
    ...
    async queryGrudges(targetUsername) {
...
//# sourceMappingURL=MemPalaceSpike.js.map