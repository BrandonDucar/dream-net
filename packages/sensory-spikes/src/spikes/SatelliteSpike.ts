...
export class SatelliteSpike implements SensorySpike {
    name = 'SatelliteSpike';
    ...
    async fetch(): Promise<SpikeResult> {
        ...
        if (useRealSensors && process.env.SATELLITE_API_KEY) {
            try {
                ...
                if (!process.env.SATELLITE_API_ENDPOINT) throw new Error("Real Sensors Enabled but no Endpoint configured");
