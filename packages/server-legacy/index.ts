...
app.use("/api", createDreamRouter());
  ...
  */
  ...
  app.use("/api/self-manage", selfManageRouter);
...
(async () => {
  // Execute safe boot sequence (health-gated startup)
  try {
    const { executeSafeBoot } = await import("./core/safe-boot");
    console.log('🚀 [Safe Boot] Starting health-gated boot sequence...');
    const bootResult = await executeSafeBoot();
    
    if (!bootResult.success) {
      console.error(`❌ [Safe Boot] Boot failed at step: ${bootResult.failedAt}`);
      ...
      console.log(`   Total duration: ${bootResult.totalDuration}ms`);
      bootResult.steps.forEach(step => {
        const status = step.success ? '✅' : '⚠️';
        console.log(`   ${status} ${step.step} (${step.duration}ms)`);
      });
    }
  } catch (error: any) {
    console.warn('[Safe Boot] Boot sequence error (continuing):', error.message);
    // Don't fail server startup if boot sequence fails
  }
