...
async function bootstrap() {
  ...
  const { secretManager } = await import('./services/SecretManagerService.js');

  await vertexAIService.initialize();

  // Quick Verification: Test Secret Manager Fallback/Fetch
  const testSecret = await secretManager.getSecret('TRIGGER_RECON');
  console.log(`[GCP] Secret Manager Validation: ${testSecret ? '✅' : '❌'} (Value Found: ${testSecret})`);

  // 1. Ignite the Spine (Nervous System)
  // 1. Ignite the Spine (Nervous System)
  const { NERVE_BUS, initNerveFabric } = await import('../../nerve/src/index.js');
  ...
  const { broadcastStarbridgeEvent } = await import('./starbridge/bus.js');

  NERVE_BUS.subscribe('*', async (event: any) => {
    // Only bridge specific high-value events to durable storage
    if (event.kind?.includes('System') || event.priority > 3) {
      await broadcastStarbridgeEvent({
        topic: 'system_log',
        source: `nerve:${event.channelId}`,
        type: event.kind,
  ...
  apiRouter.use("/chronicle", chronicleRouter);
  apiRouter.use("/bazaar", bazaarRouter);
  apiRouter.use("/ingress", (await import('./routes/ingress.js')).default); // Sovereign Ingress
  ...
  const { RealtimeBridge } = await import('./core/RealtimeBridge.js');
  const realtimeBridge = new RealtimeBridge(httpServer);
  ...
  // 5.7 Activate Neural Plasticity (Self-Healing Sentinel)
  await import('./core/NeuralPlasticityService.js');
  // 5.8 Initialize Memory DNA (Total Recall)
  await import('./core/MemoryDNA.js');

  // 5.9 Ignite Energy Spine (Phase XIX Architecture)
  await import('./core/AetherFactoryGovernor.js');
  await import('./core/EnergyTelemetry.js');
  await import('./core/PowerManagerService.js');
  await import('./core/EconomicGovernor.js');
  await import('./core/ArenaService.js');

  console.log("   - [5.9] Energy Spine Active (Aether Factory Online)");

  // 6.0 Activate Digital Sovereignty (Phase XXI)
  await import('./core/LaminarHarborService.js');
  await import('./core/VectorMeshConsolidator.js');
  await import('./core/CrewGraphExecutor.js');
  await import('./core/SSIPassportService.js');
  await import('./core/MechanisticInterpretabilityService.js');

  console.log("   - [6.0] Digital Sovereignty Online (Laminar Harbor & Vector Mesh Active)");

  // 7.0 Ignite Phase XXII: Front-End Finalization & Physical Finality
  await import('./core/MatterCompilerService.js');
  await import('./core/FinalityAnchorService.js');
  await import('./core/HmacIngressGuard.js');
  await import('./core/EventGraphQLService.js');
  await import('./core/ChilizBridge.js');

  console.log("   - [7.0] Phase XXIII Mastery Active (Svelte Substrate & Chiliz Online)");

  // 5.7 Ignite the Dutch Oven (Sovereign Solver)
  const { IntentGenerator, SolverAuditService } = await import('../../nerve/src/index.js');
  ...
  const server = httpServer.listen(port, host as any, async () => {
    ...
    const { NERVE_BUS: nerveBusInstance } = await import('../../nerve/src/index.js');
