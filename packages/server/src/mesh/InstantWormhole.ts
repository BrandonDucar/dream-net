...
class InstantWormhole {
   ...
   * Anchored to Orbital Connectivity
   */
  private async routeInstantly(event: InstantEvent): Promise<void> {
    // 🛰️ ORBITAL GATE: Check satellite status before routing
    try {
      const { cortex } = await import('../services/SensoryCortex.js');
      const snapshot = cortex.getLatestSnapshot();

      if (snapshot && snapshot.planetary?.satellite) {
        const latency = snapshot.planetary.satellite.imagery_latency_ms;
        if (latency > 800) {
          console.warn(`[InstantWormhole] 🛰️ ORBITAL INTERFERENCE DETECTED (${latency}ms). Throttling wormhole...`);
          // Simulate delay or drop depending on priority
          if (event.priority < 5) return;
        }
      }
    } catch (e) {
      // Fallback to mesh if cortex is unavailable
    }
