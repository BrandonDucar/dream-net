...
import { insertDreamSchema, insertCocoonSchema, insertDreamCoreSchema, insertWalletSchema, type ContributorRole, contributorsLog, notifications, type CocoonLog } from "@dreamnet/shared/schema";
import { requireAdmin as oldRequireAdmin, isAdminWallet } from './auth.js';
import {
  generateNonce,
  createSiweMessage,
  verifySignature,
...
import { dreamScoreEngine } from './dream-score-engine.js';
import { notificationEngine } from './notification-engine.js';
import { getWalletData, analyzeWallet, type WalletAnalysis } from '@dreamnet/shared/wallet';
import { z } from "zod";
import { db, mongoDb } from './db.js';
import gardenFeedRouter from './gardenFeedRouter.js';
import taskConnectorRouter from './routes-connector.js';
import streamlinedConnectorRouter from './routes/connector.js';
import dreamsRouter from './routes/dreams.js';
import walletScanRouter from './routes/wallet-scan.js';
import dreamProcessorRouter from './routes/dream-processor.js';
import walletScoreRouter from './routes/wallet-score.js';
import dreamCoresRouter from './routes/dream-cores.js';
import { connectorBotV1 } from './routes/ConnectorBot.js';
import lucidRoute from './routes/lucid.js';
import canvasRoute from './routes/canvas.js';
import rootRoute from './routes/root.js';
import echoRoute from './routes/echo.js';
import dreamStorageRouter from './routes/dream-storage.js';
import saveCoreRoute from './routes/save-core.js';
import loadCoreRoute from './routes/load-core.js';
import reactivateCoreRoute from './routes/reactivate-core.js';
import generateDreamLinkRoute from './routes/generate-dream-link.js';
import sharedDreamRoute from './routes/shared-dream.js';
import publicDreamRoute from './routes/public-dream.js';
import mutateDreamRoute from './routes/mutate-dream.js';
import saveMutatedDreamRoute from './routes/save-mutated-dream.js';
import loadDreamsRoute from './routes/load-dreams.js';
import allDreamsRoute from './routes/all-dreams.js';
import getDreamRoute from './routes/get-dream.js';
import getDreamByIdRoute from './routes/get-dream-by-id.js';
// Removed fuseDreamsRoute import as it's handled inline
import fusionsRoute from './routes/fusions.js';
// import { withGovernance } from '../packages/dreamnet-control-core/controlCoreMiddleware.js';
const withGovernance = (config: any) => (req: any, res: any, next: any) => next();
// import { browserAgentWrapper } from '../spine/wrappers/BrowserAgentWrapper.js';
const browserAgentWrapper = { validateParams: () => ({ valid: true }) }; // Stub
import { defaultDomainAllowlist } from './core/browser-agent/domainAllowlist.js';
import { defaultIpBlocking } from './core/browser-agent/ipBlocking.js';

// Feature Flags
const ENABLE_BROWSER_GOVERNANCE = process.env.ENABLE_BROWSER_GOVERNANCE === 'true';
const ENABLE_BROWSER_WRAPPER = process.env.ENABLE_BROWSER_WRAPPER === 'true';

// Governance Middleware (Conditional)
const browserGovernance = ENABLE_BROWSER_GOVERNANCE
  ? withGovernance({ clusterId: 'BROWSER_AGENT', requiredOfficeId: 'BROWSER_OPERATOR' })
  : (req: any, res: any, next: any) => next();

import claimFusionRoute from './routes/claim-fusion.js';
import dreamViewer from './routes/dreams.js';
import walletScoringRouter from './routes/wallet-scoring.js';
import echoScoreRouter from './routes/echo-score.js';
import mintTokenRouter from './routes/mint-dream-token.js';
import { smsRouter } from './routes/sms.js';
import mintDreamRouter from './routes/mint-dream.js';
// fuseDreamsRouter removed as route is handled inline
import myDreamsRouter from './routes/my-dreams.js';
import remixDreamRouter from './routes/remix-dream.js';
import dreamTitlesRouter from './routes/dream-titles.js';
import saveDreamRouter from './routes/save-dream.js';
import postBountyHandler from './routes/post-bounty.js';
import getBountiesHandler from './routes/get-bounties.js';
import joinDreamTeamHandler from './routes/join-dream-team.js';
import getDreamForksHandler from './routes/get-dream-forks.js';
import getDreamsByCloudHandler from './routes/get-dreams-by-cloud.js';
import evolutionVaultRouter from './routes/evolution-vault.js';
import aiSurgeonRouter from './routes/ai-surgeon.js';
import defenseNetworkRouter from './routes/defense-network.js';
import evolutionEngineRouter from './routes/evolution-engine.js';
import opsRouter from './routes/ops.js';
import adminWalletsRouter from './routes/admin-wallets.js';
import antigravityMemory from './routes/antigravity-memory.js';
import baseHealthRouter from './routes/base-health.js';
import websiteDesignerRouter from './routes/website-designer.js';
import deploymentRouter from './routes/deployment.js';
import cardForgeRouter from './routes/card-forge.js';
import domainIssuanceRouter from './routes/domain-issuance.js';
import passportsRouter from './routes/passports.js';
import citizensRouter from './routes/citizens.js';
import registerAgentsRouter from './routes/register-agents.js';
import agentMarketRouter from './routes/agent-market.js';
import awsRouter from './routes/aws.js';
import googleCloudRouter from './routes/google-cloud.js';
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.use("/api/market", agentMarketRouter);
  ...
  */
  ...
  app.post("/api/lighthouse/audit", browserGovernance, async (req, res) => {
    try {
      const { url } = req.body;
      ...
      const { lighthouseAuditor } = await import('./lighthouse-auditor.js');

      // Run the audit
      let auditResult;

      if (ENABLE_BROWSER_WRAPPER) {
        console.log(`[POST /api/lighthouse/audit] Using BrowserAgentWrapper for ${url}`);
        // Extract caller info from request (populated by identityResolver/governance)
        const callerId = (req as any).user?.id || 'anonymous';
        const tierId = (req as any).user?.tier || 'FREE';

        auditResult = await browserAgentWrapper.auditWebsite({
          url,
          callerId,
          tierId
        });
      } else {
        console.log(`[POST /api/lighthouse/audit] Using legacy LighthouseAuditor for ${url}`);
        auditResult = await lighthouseAuditor.auditWebsite(url);
      }
  ...
  app.post("/api/lighthouse/dream-audit", browserGovernance, async (req, res) => {
    try {
      ...
      const { lighthouseAuditor } = await import('./lighthouse-auditor.js');
      const { gptDreamProcessor } = await import('./gpt-dream-processor.js');

      // Run the Lighthouse audit
      let auditResult;

      if (ENABLE_BROWSER_WRAPPER) {
        console.log(`[POST /api/lighthouse/dream-audit] Using BrowserAgentWrapper for ${url}`);
        const callerId = (req as any).user?.id || 'anonymous';
        const tierId = (req as any).user?.tier || 'FREE';
        ...
        console.log(`[POST /api/lighthouse/dream-audit] Using legacy LighthouseAuditor for ${url}`);
        auditResult = await lighthouseAuditor.auditWebsite(url);
      }
  ...
  app.get("/api/lighthouse/quick-check/:encodedUrl", async (req, res) => {
    try {
      ...
      // Security Checks (if wrapper enabled)
      if (ENABLE_BROWSER_WRAPPER) {
        const allowlistCheck = defaultDomainAllowlist.isAllowed(url);
        ...
        const ipCheck = await defaultIpBlocking.validateUrl(url);
        if (!ipCheck.allowed) {
          return res.status(403).json({
            error: 'Access denied',
  ...
  app.post("/api/webhooks/test", requireAdmin, async (req, res) => {
    try {
      const { webhookNotifier } = await import('./webhook-notifier.js');
  ...
  app.post("/api/test/orchestration", requireAdmin, async (req, res) => {
    try {
      ...
      import('./orchestration-script.js').then(async (module) => {
        try {
  ...
  app.post("/api/evaluate-dream/:id", requireAdmin, async (req, res) => {
    try {
      ...
      const { dreamEvaluator } = await import('./ai-dream-evaluator.js');
  ...
  app.post("/api/evaluate-all-dreams", requireAdmin, async (req, res) => {
    try {
      ...
      import('./ai-dream-evaluator.js').then(async (module) => {
        try {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    app.post("/api/dev/generate-dreams", async (req, res) => {
      try {
        const { generateBatch } = await import('./dev-test-generator.js');
    ...
    app.post("/api/dev/run-orchestration", async (req, res) => {
      try {
        const { runOrchestration } = await import('./orchestration-script.js');
    ...
    app.post("/api/dev/run-test", async (req, res) => {
      try {
        const { runDemoTest } = await import('../test-runner.js');
        await runDemoTest();
  ...
  app.get("/api/simple-notifications", async (req, res) => {
    try {
      ...
      const { simpleNotifications } = await import('./simple-notifications.js');
      const limit = parseInt(req.query.limit as string) || 20;
  ...
  app.use('/api/antigravity-memory', antigravityMemory);
  ...
  app.post('/api/migrate-dream-cloud', async (req, res) => {
    try {
      const { migrateDreamCloud } = await import('./migrate-dream-cloud.js');
