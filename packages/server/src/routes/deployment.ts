...
router.post('/deploy', async (req, res) => {
  try {
    ...
    if (result.success) {
      // Monetize the sprawl
      grantReward("system-deploy-engine", "deployment-success", {
        reason: `Successful Sprawl: ${config.projectName} to ${config.platform}`,
        meta: { projectName: config.projectName, platform: config.platform, deploymentId: result.deploymentId }
      }).catch(err => console.error("Deployment reward failed:", err));
    }
