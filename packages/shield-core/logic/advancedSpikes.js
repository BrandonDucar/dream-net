 ...
 */
export function fireAdvancedSpike(name, type, target, power = 1.0, meta) {
    const spike = {
        id: nextSpikeId(),
        name,
 ...
 */
function executeAdvancedSpike(spike) {
 ...
 */
export function fireAdvancedSpikeAtThreat(threat, spikeType) {
    // Determine spike type based on threat
    let type = spikeType || "automated-response";
    if (!spikeType) {
        switch (threat.level) {
            case "critical":
            case "extreme":
    ...
    return fireAdvancedSpike(`Advanced response to ${threat.type}`, type, threat.source || threat.target || "unknown", power, { threatId: threat.id, threatType: threat.type, threatLevel: threat.level });
}
/**
 * Track spike effectiveness and learn which spikes work best
 */
export function trackSpikeEffectiveness(spike, threatNeutralized) {
 ...
 */
export function getBestSpikeType(threat) {
    ...
    const effectivenessByType = new Map();
    ...
    let bestType = "automated-response";
