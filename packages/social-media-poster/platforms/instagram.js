...
export class InstagramPoster {
    ...
    async postPhoto(imageUrl, caption) {
        try {
            ...
            const createResponse = await axios.post(`${this.baseUrl}/${this.config.instagramBusinessAccountId}/media`, {
                image_url: imageUrl,
                caption: caption,
            ...
            const publishResponse = await axios.post(`${this.baseUrl}/${this.config.instagramBusinessAccountId}/media_publish`, {
                creation_id: creationId,
                access_token: this.config.accessToken,
            });
            const mediaId = publishResponse.data.id;
            // Step 3: Get permalink
            const permalinkResponse = await axios.get(`${this.baseUrl}/${mediaId}`, {
                params: {
                    fields: "permalink",
                    access_token: this.config.accessToken,
                },
     ...
     */
    async postVideo(videoUrl, caption, thumbnailUrl) {
        try {
            // Step 1: Create media container
            const createResponse = await axios.post(`${this.baseUrl}/${this.config.instagramBusinessAccountId}/media`, {
