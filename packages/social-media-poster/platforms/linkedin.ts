...
export interface LinkedInConfig {
  ...
  personUrn?: string; // User's LinkedIn URN (optional, can be fetched)
}
...
export class LinkedInPoster {
   ...
   */
  async post(content: string): Promise<{ success: boolean; postId?: string; permalink?: string; error?: string }> {
    try {
   ...
   * Post with image
   */
  async postWithImage(content: string, imageUrl: string): Promise<{ success: boolean; postId?: string; permalink?: string; error?: string }> {
    try {
      // Step 1: Upload image
