 ...
 */
export function learnThreadPatterns() {
    ...
    for (const [kind, threads] of threadsByKind.entries()) {
        ...
        const existingPattern = Array.from(SpiderStore.listThreadPatterns()).find((p) => p.threadKind === kind);
        if (existingPattern) {
 ...
 */
export function learnFlyPatterns() {
    ...
    for (const [key, flies] of fliesByTypeSource.entries()) {
        ...
        // Check if pattern already exists
        const existingPattern = Array.from(SpiderStore.listFlyPatterns()).find((p) => p.flyType === flyType && p.source === source);
        if (existingPattern) {
            // Update existing pattern
