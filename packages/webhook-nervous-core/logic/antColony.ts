 ...
 */
export function findBestTrail(destination: string): PheromoneTrail | null {
  const relevantTrails = Array.from(pheromoneTrails.values()).filter(
    (trail) => trail.path[trail.path.length - 1] === destination && trail.strength > 0.1
  );

  if (relevantTrails.length === 0) return null;
 ...
 */
export function followTrail(antId: string, trailId: string): boolean {
 ...
 */
export function completeAnt(antId: string, success: boolean, latency: number) {
  const ant = activeAnts.get(antId);
  if (!ant) return;
 ...
 */
export function getPheromoneTrails(): PheromoneTrail[] {
 ...
 * Get all active ants
 */
export function getActiveAnts(): WebhookAnt[] {
 ...
 */
export function getStuckAnts(timeoutMs: number = 30000): WebhookAnt[] {
  const now = Date.now();
  return Array.from(activeAnts.values()).filter(
    (ant) => now - ant.startedAt > timeoutMs && ant.status !== "completed"
  );
}
 ...
 * Mark ant as stuck
 */
export function markAntStuck(antId: string) {
  const ant = activeAnts.get(antId);
  if (ant) {
    ant.status = "stuck";
