 ...
 */
export function autoDiscoverWebhooksFromEnv(): WebhookNeuron[] {
  const discovered: WebhookNeuron[] = [];
  ...
  for (const [key, value] of Object.entries(process.env)) {
    ...
    if (isWebhook) {
      try {
        ...
        let type: WebhookNeuron["type"] = "motor"; // Outgoing by default
 ...
 */
export function autoDiscoverWebhooksFromConfig(): WebhookNeuron[] {
  const discovered: WebhookNeuron[] = [];
 ...
 */
function discoverFromConfigObject(config: any, discovered: WebhookNeuron[]): void {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (typeof value === "string" && value.startsWith("http")) {
        ...
        const name = fullKey
          ...
          .replace(/[._-]/g, " ")
          .trim()
          .replace(/\b\w/g, (l: string) => l.toUpperCase()) || "Webhook";
 ...
 */
export function autoDiscoverAllWebhooks(): WebhookNeuron[] {
  const discovered: WebhookNeuron[] = [];
  ...
  const unique = new Map<string, WebhookNeuron>();
...
export function autoCreateDefaultAntibodies() {
  ...
  createAntibody(
    ...
    "block",
    { memory: true, effectiveness: 0.95 }
  );
  ...
  createAntibody(
    ...
    "block",
    { memory: true, effectiveness: 0.9 }
  );

  // Rate limit detection
  createAntibody(
    "Rate Limit Detector",
    "(?i)(rate.?limit|too.?many|429)",
    "alert",
    { memory: true, effectiveness: 0.8 }
  );

  // Timeout detection
  createAntibody(
    "Timeout Detector",
    "(?i)(timeout|timed.?out|504|408)",
    "alert",
    { memory: true, effectiveness: 0.85 }
  );
