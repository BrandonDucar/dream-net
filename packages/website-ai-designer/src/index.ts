...
export class WebsiteAIDesigner {
   ...
   */
  async generateWebsite(request: WebsiteDesignRequest): Promise<WebsiteDesignResult> {
    if (!this.openai) {
      return {
        success: false,
   ...
   * Build prompt for website generation
   */
  private buildWebsitePrompt(request: WebsiteDesignRequest): string {
   ...
   */
  private parseWebsiteResponse(content: string, request: WebsiteDesignRequest): WebsiteDesignResult {
   ...
   */
  async generateWebsiteCode(request: WebsiteDesignRequest): Promise<{
    html: string;
    css: string;
    js: string;
    instructions: string;
  ...
  private generateHTML(request: WebsiteDesignRequest, result: WebsiteDesignResult): string {
...
<body>
  ...
  private generateCSS(request: WebsiteDesignRequest): string {
    return `* {
    margin: 0;
    padding: 0;
...
@media (max-width: 768px) {
  ...
  private generateJS(request: WebsiteDesignRequest): string {
...
console.log('Website loaded successfully');`;
  ...
  private generateDeploymentInstructions(result: WebsiteDesignResult): string {
...
let designerInstance: WebsiteAIDesigner | null = null;

export function getWebsiteDesigner(): WebsiteAIDesigner {
  if (!designerInstance) {
