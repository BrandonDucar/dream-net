 ...
 */
export function scoreLead(ctx, lead) {
    ...
    if (ctx.reputationLattice?.status) {
        try {
            ...
            if (repStatus.scoresSample) {
                const leadRep = repStatus.scoresSample.find((s) => s.entityId === lead.id || s.entityId === lead.name.toLowerCase());
    ...
    const typeBoost = lead.type === "ecosystem-fund" || lead.type === "grant" || lead.type === "accelerator" ? 0.2 : 0;
    const tagBoost = (tags.includes("base") ? 0.1 : 0) +
        (tags.includes("infra") ? 0.1 : 0) +
        (tags.includes("ai") ? 0.1 : 0);
    const priority = priorityScore;
    const trust = trustScore;
    // combine into hotScore
    const hotScore = clamp01(0.4 * priority +
        0.3 * trust +
