...
var FIGMA_TOKEN = process.env.FIGMA_ACCESS_TOKEN;
...
async function syncFigma() {
  console.log(`\u{1F3A8} Starting Figma Syncope for File: ${FIGMA_FILE_KEY}...`);
  try {
...
`;
  ...
  cssContent += "}";
  const cssPath = import_path.default.resolve(process.cwd(), "packages/organs/integumentary/client/src/index.css");
  let existingCss = "";
  if (import_fs.default.existsSync(cssPath)) {
    existingCss = import_fs.default.readFileSync(cssPath, "utf-8");
  }
...
/* FIGMA_SYNC_END */`;
  ...
  import_fs.default.writeFileSync(cssPath, updatedCss);
  console.log("\u{1F680} index.css updated with Figma tokens!");
}
syncFigma();
