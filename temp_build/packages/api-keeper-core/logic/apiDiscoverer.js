...
exports.discoverAPIs = discoverAPIs;
...
function discoverAPIs() {
    ...
    for (const provider of discovered) {
        const existing = apiStore_1.APIStore.getProvider(provider.id);
        if (!existing) {
            apiStore_1.APIStore.addProvider(provider);
            console.log(`[APIDiscoverer] Discovered provider: ${provider.name} (${provider.category})`);
        }
        else {
            // Update last checked
            apiStore_1.APIStore.updateProvider(provider.id, { lastCheckedAt: Date.now() });
        }
    }
    return discovered;
}
 ...
 */
function searchProviders(category, feature) {
    let results = apiStore_1.APIStore.listProviders();
    if (category) {
