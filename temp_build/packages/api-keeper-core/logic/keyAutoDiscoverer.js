...
exports.autoDiscoverKeysFromEnv = autoDiscoverKeysFromEnv;
exports.scanEnvFile = scanEnvFile;
exports.scanConfigFiles = scanConfigFiles;
exports.scanVercelEnv = scanVercelEnv;
exports.autoDiscoverAllKeys = autoDiscoverAllKeys;
exports.continuousAutoDiscovery = continuousAutoDiscovery;
 ...
 */
function autoDiscoverKeysFromEnv() {
    const discovered = [];
    ...
    for (const [envKey, configs] of Object.entries(ENV_PATTERNS)) {
        ...
        for (const config of configs) {
            const existing = apiStore_1.APIStore.listKeysForProvider(config.providerId);
            const alreadyRegistered = existing.some((k) => k.key === value || k.name.includes(config.name));
            if (!alreadyRegistered) {
                ...
                if (config.providerId === "twilio" && envKey === "TWILIO_ACCOUNT_SID") {
                    ...
                    if (authToken) {
                        const key = (0, keyManager_1.registerKey)(config.providerId, value, authToken, {
                    ...
                    const key = (0, keyManager_1.registerKey)(config.providerId, value, undefined, {
                        name: `Auto-discovered ${config.name}`,
                        tags: ["auto-discovered", "env", "zero-touch"],
                    });
                    discovered.push(key);
    ...
    for (const [envKey, value] of Object.entries(process.env)) {
        if (!value || value.length < 10)
            continue; // Skip short values
        if (registeredProviders.has(envKey.split("_")[0].toLowerCase()))
        ...
        for (const { pattern, providerHint } of GENERIC_PATTERNS) {
            ...
            if (match) {
                ...
                const provider = providers.find((p) => p.id === hint ||
                    p.id.includes(hint) ||
                    p.name.toLowerCase().includes(hint) ||
                    hint.includes(p.id) ||
                    hint.includes(p.name.toLowerCase().replace(/\s+/g, "-")));
                if (provider) {
                    const existing = apiStore_1.APIStore.listKeysForProvider(provider.id);
                    const alreadyRegistered = existing.some((k) => k.key === value);
                    if (!alreadyRegistered) {
                        const key = (0, keyManager_1.registerKey)(provider.id, value, undefined, {
                    ...
                    const key = (0, keyManager_1.registerKey)(hint, value, undefined, {
 ...
 */
function autoDiscoverAllKeys() {
    const discovered = [];
    const allDiscoveredKeys = new Set(); // Track to avoid duplicates
        ...
        if (!process.env[key]) {
            ...
            if (provider) {
                ...
                if (!existing.some((k) => k.key === value)) {
                    const registered = (0, keyManager_1.registerKey)(provider.id, value, undefined, {
 ...
 */
function continuousAutoDiscovery() {
    // Run full discovery
