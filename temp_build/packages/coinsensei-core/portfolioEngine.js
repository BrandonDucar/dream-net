...
class PortfolioEngine {
    ...
    async computePortfolio(manualEntries, cexTransactions = [], userPrices) {
        ...
        // Fetch prices
        const tokenIds = Array.from(tokenGroups.keys()).map(symbol => priceProvider_1.PriceProvider.getCoinGeckoId(symbol));
        ...
        for (const [token, trades] of tokenGroups.entries()) {
            const tokenId = priceProvider_1.PriceProvider.getCoinGeckoId(token);
