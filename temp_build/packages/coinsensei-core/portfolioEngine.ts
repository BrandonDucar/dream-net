...
export class PortfolioEngine {
  ...
  async computePortfolio(
  ...
  ): Promise<{ positions: TokenPosition[]; summary: PortfolioSummary }> {
    // Combine all trades
    ...
    // Fetch prices
    const tokenIds = Array.from(tokenGroups.keys()).map(symbol =>
      PriceProvider.getCoinGeckoId(symbol)
    );
  ...
  private computeFIFO(buyTrades: Trade[], sellTrades: Trade[]): number {
  ...
  private computeWinLossRatio(buyTrades: Trade[], sellTrades: Trade[]): number {
  ...
  private generateSummary(
    ...
    const concentrationWarnings = positions.filter(
      p => p.allocation_pct >= this.config.concentration_warn_threshold
    );
