...
exports.SmartModeEngine = void 0;
class SmartModeEngine {
    concentrationThreshold;
    constructor(concentrationThreshold = 25) {
        this.concentrationThreshold = concentrationThreshold;
    }
    generateDCASuggestions(positions, trendData) {
        const suggestions = [];
    ...
    generateRebalanceSuggestions(positions, targetCap) {
        const suggestions = [];
    ...
    detectAnomalies(positions, summary, previousSummary) {
        const anomalies = [];
        // Detect PnL spikes
        if (previousSummary) {
        ...
        if (previousSummary) {
            const newTokens = positions.filter(p => !previousSummary.top_holdings.some(h => h.token === p.token) && p.total_value > 100);
...
exports.SmartModeEngine = SmartModeEngine;
