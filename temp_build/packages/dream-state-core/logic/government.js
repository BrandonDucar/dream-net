...
exports.ensureGovernmentDepartments = ensureGovernmentDepartments;
exports.recordGovernmentAction = recordGovernmentAction;
exports.ensureStateSymbols = ensureStateSymbols;
 ...
 */
function ensureGovernmentDepartments() {
    const existing = citizenshipStore_1.CitizenshipStore.listDepartments();
    ...
    for (const dept of departments) {
        citizenshipStore_1.CitizenshipStore.createDepartment(dept);
    }
    return departments;
}
/**
 ...
 */
function recordGovernmentAction(type, department, action, meta) {
    const govAction = {
    ...
    citizenshipStore_1.CitizenshipStore.recordAction(govAction);
    return govAction;
}
/**
 * Initialize state symbols
 */
function ensureStateSymbols() {
    const existing = citizenshipStore_1.CitizenshipStore.listSymbols();
    if (existing.length > 0)
        return existing;
    const now = Date.now();
    const symbols = [
...
DreamNet, DreamNet, forever alive`,
    ...
    for (const symbol of symbols) {
        citizenshipStore_1.CitizenshipStore.adoptSymbol(symbol);
    }
    return symbols;
}
