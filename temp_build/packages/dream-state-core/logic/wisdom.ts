...
import { Pulsar } from "@dreamnet/memory-dna/logic/pulsar";
...
export class WisdomEngine {
    ...
    static async judgeProposal(proposal: DreamProposal): Promise<WisdomAssessment> {
        ...
        const precedents = await vectorStore.query(`governance precedent similar to: ${proposal.title} ${proposal.description}`, 3);
