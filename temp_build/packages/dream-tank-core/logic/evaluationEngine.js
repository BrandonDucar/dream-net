...
exports.evaluateDream = evaluateDream;
...
function evaluateDream(ctx, dream, kind = "health-check") {
    const now = Date.now();
    ...
    const score = Math.max(0, Math.min(1, 0.5 * effectivePriority +
        0.3 * effectiveTrust +
        0.2 * (1 - effectiveRisk)));
    const summary = `Dream '${dream.name}' evaluated with score=${score.toFixed(2)}, priority=${effectivePriority.toFixed(2)}, trust=${effectiveTrust.toFixed(2)}, risk=${effectiveRisk.toFixed(2)}.`;
    const evaluation = {
        id: nextEvalId(),
        dreamId: dream.id,
        kind,
    ...
    tankStore_1.TankStore.addEvaluation(evaluation);
    return evaluation;
}
