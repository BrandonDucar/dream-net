...
function computeNextStage(stage: DreamStage, score: number): DreamStage {
  if (score < 0.3) return stage; // too weak to move
  switch (stage) {
    case "seed":
...
function computeHealth(current: string, score: number): any {
  if (score < 0.2) return "infected";
  if (score < 0.4) return "stalled";
