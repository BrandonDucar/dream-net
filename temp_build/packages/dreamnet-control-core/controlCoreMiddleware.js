...
exports.setGlobalKillSwitch = setGlobalKillSwitch;
exports.isGlobalKillSwitchEnabled = isGlobalKillSwitchEnabled;
exports.controlCoreMiddleware = controlCoreMiddleware;
...
const eventFabric_1 = require("./eventFabric");
const policyEngine_1 = require("./policyEngine");
const bus_js_1 = require("../nerve/src/bus.js");
const factory_js_1 = require("../nerve/src/factory.js");
 ...
 */
function setGlobalKillSwitch(enabled) {
 ...
 */
function hasOffice(identity, officeId) {
    if (!identity)
        return false;
...
function controlCoreMiddleware(req, res, next) {
    ...
    if (globalKillSwitch) {
        ...
        if (!isGodVault) {
            ...
            (0, eventFabric_1.emitControlCoreEvent)({
                traceId,
                callerIdentity,
                clusterId,
                routeId,
    ...
    const clusterConfig = (0, clusters_1.getClusterConfig)(clusterId);
    if (!clusterConfig) {
        baseDecision = "deny";
    ...
    if (clusterConfig.requiredFeatureFlag && !isGodVault) {
        ...
        if (!hasFeature) {
            ...
            try {
                ...
                const nerveEvent = (0, factory_js_1.createRequestDecisionEvent)({
                    traceId,
                    clusterId,
                    tierId: callerIdentity.tierId,
                ...
                bus_js_1.NERVE_BUS.publish(nerveEvent);
            }
            catch (error) {
    ...
    if (callerIdentity && !isGodVault) {
        const tierConfig = tierConfig_1.TIERS[callerIdentity.tierId];
        ...
        const rateLimitResult = (0, rateLimiter_1.checkAndConsume)(clusterId, callerIdentity.tierId, effectiveRateLimit);
        ...
        const tierConfig = callerIdentity ? tierConfig_1.TIERS[callerIdentity.tierId] : null;
        const clusterLimit = clusterConfig.defaultMaxRequestsPerMinute;
        const tierLimit = tierConfig?.maxRequestsPerMinute || 1000;
        effectiveRateLimit = Math.min(clusterLimit, tierLimit);
        // Still check rate limit for stats, but don't block
        const rateLimitResult = (0, rateLimiter_1.checkAndConsume)(clusterId, callerIdentity?.tierId || "GOD_MODE", effectiveRateLimit);
        remainingRateLimit = rateLimitResult.remaining;
        if (process.env.DEBUG_RATE_LIMIT === "true") {
    ...
    const policyResult = (0, policyEngine_1.evaluatePolicy)(policyContext);
 ...
 */
function withGovernance(options) {
    return (req, _res, next) => {
        req.clusterId = options.clusterId;
        if (options.requiredOfficeId) {
            req.requiredOfficeId = options.requiredOfficeId;
        }
        if (options.requiredCabinetId) {
            req.requiredCabinetId = options.requiredCabinetId;
        }
        next();
    };
}
