...
class RateLimiter {
     ...
     */
    checkAndConsume(clusterId, tierId, limitPerMinute) {
        ...
        if (now - window.lastCleanup > this.CLEANUP_INTERVAL) {
            window.requests = window.requests.filter((req) => req.timestamp > oneMinuteAgo);
            window.lastCleanup = now;
        }
        // Count requests in the last minute
        const recentRequests = window.requests.filter((req) => req.timestamp > oneMinuteAgo).length;
 ...
 * ```
 */
function checkAndConsume(clusterId, tierId, limitPerMinute) {
    return exports.rateLimiter.checkAndConsume(clusterId, tierId, limitPerMinute);
}
