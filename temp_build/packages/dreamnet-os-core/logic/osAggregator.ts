...
export function buildOSSnapshot(ctx: DreamNetOSContext): DreamNetOSSnapshot {
  const now = Date.now();

  const version: DreamNetVersionInfo = {
    major: 0,
    minor: 4,
    patch: 0,
  ...
  const push = (
    ...
    let status: SubsystemSummary["status"] = "unknown";
    if (ok === true) status = "ok";
    else if (ok === false) status = "warn";
  ...
  if (ctx.dreamBetCore?.status) {
    ...
    push(
      "DreamBetCore",
      ok,
      `Games=${st.gameCount}, Rounds=${st.roundCount}`,
      st.lastRunAt ?? undefined
    );
  } else {
  ...
  if (ctx.zenGardenCore?.status) {
    ...
    push(
      "ZenGardenCore",
      ok,
      `Sessions=${st.sessionCount}, Rewards=${st.rewardCount}`,
      st.lastRunAt ?? undefined
    );
  } else {
  ...
  if (ctx.dreamTankCore?.status) {
    ...
    push(
      "DreamTankCore",
      ok,
      `Dreams=${st.dreamCount}, Evaluations=${st.evaluationCount}`,
      st.lastRunAt ?? undefined
    );
  } else {
  ...
  if (ctx.liquidityEngine?.status) {
    ...
    push(
      "LiquidityEngine",
      ok,
      `Pools=${st.poolCount}, Planned=${st.plannedCount}, Active=${st.activeCount}`,
      st.lastRunAt ?? undefined
    );
  } else {
  ...
  if (ctx.socialHubCore?.status) {
    ...
    push(
      "SocialHubCore",
      ok,
      `Posts=${st.postCount}, Comments=${st.commentCount}`,
      st.lastRunAt ?? undefined
    );
  } else {
  ...
  if (ctx.economicEngineCore?.status) {
    ...
    push(
      "EconomicEngineCore",
      ok,
      `Tokens=${st.tokenCount}, Balances=${st.balanceCount}, Rewards=${st.appliedRewardCount}`,
      st.lastRunAt ?? undefined
    );
  } else {
  ...
  if (ctx.agentRegistryCore?.status) {
    ...
    push(
      "AgentRegistryCore",
      ok,
      `Agents=${st.agentCount}, Errors=${st.errorCount}`,
      st.lastRunAt ?? undefined
    );
  } else {
  ...
  if (ctx.initRitualCore?.status) {
    ...
    push(
      "InitRitualCore",
      ok,
      `Active=${st.activeIdentityCount}, Completed=${st.completedCount}`,
      st.lastRunAt ?? undefined
    );
  } else {
  ...
  if (ctx.apiKeeperCore?.status) {
    ...
    push(
      "APIKeeperCore",
      ok,
      `Providers=${st.providerCount}, Keys=${st.keyCount}, Active=${st.activeKeyCount}, Cost=$${st.costToday.toFixed(2)}`,
      st.lastRunAt ?? undefined
    );
  } else {
  ...
  if (ctx.aiSeoCore?.status) {
    ...
    push(
      "AISEOCore",
      ok,
      `Optimizations=${st.optimizationCount}, Keywords=${st.keywordCount}, Geofences=${st.geofenceCount}`,
      st.lastRunAt ?? undefined
    );
  } else {
  ...
  if (ctx.shieldCore?.status) {
    ...
    push(
      "ShieldCore",
      ok,
      `Status=${st.status}, Layers=${st.layerCount}, Threats=${st.activeThreats}, Neutralized=${st.neutralizedThreats}`,
      st.lastRunAt ?? undefined
    );
  } else {
  ...
  if (ctx.wolfPackFundingCore?.status) {
    ...
    push(
      "WolfPackFundingCore",
      ok,
      `Leads=${st.totalLeads}, Active=${st.activeLeads}, Grants=${st.grantsDrafted}`,
      st.lastRunAt ?? undefined
    );
  } else {
  ...
  if (ctx.dreamStateCore?.status) {
    ...
    push(
      "DreamStateCore",
      ok,
      `Departments=${st.departmentCount}, Proposals=${st.proposalCount}, Actions=${st.recentActionCount}`,
      st.lastRunAt ?? undefined
    );
  } else {
  ...
  if (ctx.spiderWebCore?.status) {
    ...
    push(
      "SpiderWebCore",
      ok,
      `Threads=${st.threadCount}, Active=${st.activeThreadCount}`,
      st.lastRunAt ?? undefined
    );
  } else {
  ...
  if (ctx.starBridgeLungs?.status) {
    ...
    push(
      "StarBridgeLungs",
      ok,
      `Chains=${st.chainMetrics.length}, Breaths=${st.lastBreaths.length}, LastRun=${st.lastRunAt ? new Date(st.lastRunAt).toISOString() : "never"}`,
      st.lastRunAt ?? undefined
    );
  } else {
  ...
  if (ctx.webhookNervousCore?.status) {
    ...
    push(
      "WebhookNervousCore",
      ok,
      `Neurons=${st.neurons.total}, Synapses=${st.synapses.total}, Networks=${st.mycelium.networks}`,
      st.lastRunAt ?? undefined
    );
  } else {
  ...
  if (ctx.jaggyCore?.status) {
    ...
    push(
      "JaggyCore",
      ok,
      `Status=${st.status}, Fame=${st.baseFame.toFixed(1)}, Hunts=${st.hunts.length}, Territories=${st.territories.length}`,
      st.lastHuntAt ?? undefined
    );
  } else {
  ...
  if (ctx.dreamSnailCore?.status) {
    ...
    push(
      "DreamSnailCore",
      ok,
      `Trails=${st.totalTrails}, Identities=${st.totalIdentities}, Privacy=${st.privacyScore}%`,
      st.lastTrailAt > 0 ? st.lastTrailAt : undefined
    );
  } else {
  ...
  if (typeof (global as any).DREAMKEEPER_CORE !== "undefined") {
    const dk = (global as any).DREAMKEEPER_CORE;
    if (dk.getStatus) {
      ...
      push(
        "DREAMKEEPER_CORE",
        ok,
        `Health=${st.networkVitals.networkHealth}, Dreams=${st.networkVitals.totalDreams}, Threat=${st.networkVitals.threatLevel}`,
        undefined
      );
    }
  }
  ...
  if (typeof (global as any).DreamDefenseNet !== "undefined") {
    const dd = (global as any).DreamDefenseNet;
    if (dd.status) {
      const ok = dd.status === "armed" || dd.status === "standby";
      push(
        "DreamDefenseNet",
        ok,
        `Status=${dd.status}, Threats=${dd.activeThreats}, Neutralized=${dd.neutralizedThreats}`,
        undefined
      );
    }
  }
  ...
  if (typeof (global as any).SurgeonAgent !== "undefined") {
    const sa = (global as any).SurgeonAgent;
    if (sa.getStatus) {
      ...
      push(
        "SurgeonAgent",
        ok,
        `Status=${st.status}, Tasks=${st.taskQueue.length}, Completed=${st.completedTasks}`,
        undefined
      );
    }
  }
  ...
  if (typeof (global as any).EvolutionEngine !== "undefined") {
    const ee = (global as any).EvolutionEngine;
    if (ee.getStatus) {
      ...
      push(
        "EvolutionEngine",
        ok,
        `Status=${st.status}, Cycles=${st.totalCycles}, Upgrades=${st.totalUpgrades}`,
        undefined
      );
    }
  }
  ...
  const economyHealth = clamp01(
    (subsystems.find((s) => s.name === "EconomicEngineCore" && s.status === "ok")
      ? 0.7
      : 0.4) +
  ...
  const socialHealth = clamp01(
    subsystems.find((s) => s.name === "SocialHubCore" && s.status === "ok")
      ? 0.8
      : 0.4
  );

  const dreamPipelineHealth = clamp01(
    (subsystems.find((s) => s.name === "DreamTankCore" && s.status === "ok")
      ? 0.6
      : 0.3) +
  ...
  const globalHealth: GlobalHealthScores = {
    infraHealth,
    economyHealth,
    socialHealth,
    dreamPipelineHealth,
