...
exports.analyzeCleanup = analyzeCleanup;
exports.executeCleanupAction = executeCleanupAction;
exports.executeCleanup = executeCleanup;
const vercelClient_1 = require("./vercelClient");
const dreamnet_operational_bridge_1 = require("../../dreamnet-operational-bridge");
/**
 * Analyze projects and find cleanup opportunities
 */
async function analyzeCleanup(targetDomain = "dreamnet.ink") {
    const actions = [];
    try {
        ...
        for (const project of projects) {
            const deployments = await (0, vercelClient_1.listDeployments)(project.id, 50);
            ...
            const domains = await (0, vercelClient_1.getProjectDomains)(project.id);
    ...
    try {
        switch (action.type) {
            case "delete_deployment":
                const deleted = await (0, vercelClient_1.deleteDeployment)(action.target);
                if (deleted) {
                ...
                return deleted;
            case "delete_project":
                const projectDeleted = await (0, vercelClient_1.deleteProject)(action.target);
                if (projectDeleted) {
                    (0, dreamnet_operational_bridge_1.bridgeToSpiderWeb)({
                        type: "audit_event",
                        severity: "medium",
            ...
            case "update_domain":
                ...
                if (domainAction === "add" && domain) {
                    const added = await (0, vercelClient_1.addDomain)(projectId, domain);
                    if (added) {
                        (0, dreamnet_operational_bridge_1.bridgeToSpiderWeb)({
                            type: "audit_event",
                            severity: "low",
                    ...
                    const removed = await (0, vercelClient_1.removeDomain)(projectId, domain);
                    if (removed) {
                        (0, dreamnet_operational_bridge_1.bridgeToSpiderWeb)({
                            type: "audit_event",
                            severity: "low",
 ...
 */
async function executeCleanup(actions, dryRun = true) {
    let executed = 0;
    let failed = 0;
    let skipped = 0;
