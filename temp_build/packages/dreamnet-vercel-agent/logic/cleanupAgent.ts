 ...
 */
export async function analyzeCleanup(targetDomain: string = "dreamnet.ink"): Promise<CleanupAction[]> {
  ...
  try {
    ...
    const projectGroups = new Map<string, VercelProject[]>();
    for (const project of projects) {
 ...
 */
export async function executeCleanupAction(action: CleanupAction): Promise<boolean> {
  try {
    switch (action.type) {
