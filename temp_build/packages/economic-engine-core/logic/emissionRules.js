...
exports.ensureDefaultEconomicConfigSeeded = ensureDefaultEconomicConfigSeeded;
const econStore_1 = require("../store/econStore");
let initialized = false;
/**
 * Seed default token configs and emission rules if they don't exist yet.
 */
function ensureDefaultEconomicConfigSeeded() {
    if (initialized)
        return;
    // Token configs
    const tokenConfigs = [
    ...
    tokenConfigs.forEach((cfg) => econStore_1.EconStore.upsertTokenConfig(cfg));
    // Emission rules
    const rules = [
    ...
    rules.forEach((r) => econStore_1.EconStore.addEmissionRule(r));
    initialized = true;
}
