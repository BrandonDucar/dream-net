 ...
 */
export function discoverFromProcessEnv(): EnvVariable[] {
  const discovered: EnvVariable[] = [];
  const knownPatterns: Record<string, { category: EnvCategory; isSecret: boolean; description?: string }> = {
    // API Keys
 ...
 */
export function discoverFromEnvFiles(): EnvVariable[] {
  const discovered: EnvVariable[] = [];
  ...
  try {
    ...
    function findEnvFiles(dir: string, depth: number = 0) {
      ...
      try {
        ...
        for (const line of lines) {
          ...
          if (key && value && value.length > 0) {
            ...
            const category: EnvCategory = 
              keyTrimmed.includes("DB") || keyTrimmed.includes("DATABASE") || keyTrimmed.includes("POSTGRES") ? "database" :
 ...
 */
export async function discoverAllEnvVars(): Promise<EnvVariable[]> {
  const discovered: EnvVariable[] = [];
