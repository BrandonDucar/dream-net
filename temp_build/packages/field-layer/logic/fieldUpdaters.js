...
exports.updateFieldsFromReputation = updateFieldsFromReputation;
exports.updateFieldsFromStarBridge = updateFieldsFromStarBridge;
exports.updateFieldsFromQAL = updateFieldsFromQAL;
exports.updateFieldsFromDreamCortex = updateFieldsFromDreamCortex;
exports.updateFieldsFromWolfPackAndPSL = updateFieldsFromWolfPackAndPSL;
const fieldStore_1 = require("../store/fieldStore");
function point(kind, id) {
    return { kind, id };
}
function updateFieldsFromReputation(ctx, now) {
    ...
    scores.forEach((s) => {
        ...
        fieldStore_1.FieldStore.upsertSample("trust", p, trustValue, now);
        fieldStore_1.FieldStore.upsertSample("risk", p, riskValue, now);
    });
}
function updateFieldsFromStarBridge(ctx, now) {
    ...
    metrics.forEach((m) => {
        ...
        const reliability = typeof m.reliability === "number"
            ? m.reliability
            : 0.5;
        fieldStore_1.FieldStore.upsertSample("liquidity", p, liquidity, now);
        fieldStore_1.FieldStore.upsertSample("trust", p, reliability, now);
        fieldStore_1.FieldStore.upsertSample("risk", p, 1 - reliability, now);
    });
}
function updateFieldsFromQAL(ctx, now) {
    ...
    fieldStore_1.FieldStore.upsertSample("load", p, 0.5, now);
}
function updateFieldsFromDreamCortex(ctx, now) {
    ...
    dreams.forEach((d) => {
        ...
        fieldStore_1.FieldStore.upsertSample("dreamPriority", p, priorityValue, now);
    });
}
function updateFieldsFromWolfPackAndPSL(ctx, now) {
    ...
    if (ctx.wolfPack?.status) {
        ...
        activeTargets.forEach((id) => {
            ...
            fieldStore_1.FieldStore.upsertSample("risk", p, 0.8, now);
        });
    }
    ...
    if (ctx.predatorScavenger?.status) {
        ...
        decaySignals.forEach((sig) => {
            ...
            fieldStore_1.FieldStore.upsertSample("risk", p, sev, now);
        });
    }
}
