 ...
 */
async function fuseAgents(agentA, agentB, fusionType = "temporary") {
    const hybridId = `hybrid-${(0, node_crypto_1.randomUUID)().slice(0, 8)}`;
    // MERGE TRAITS
    ...
    await VectorStore_1.vectorStore.addMemory(`FUSION EVENT: ${agentA.entityId} fused with ${agentB.entityId} to create ${hybrid.name}`, { type: "agent_fusion", hybridId, parentIds: hybrid.parentIds, traits: newTraits });
    return hybrid;
}
