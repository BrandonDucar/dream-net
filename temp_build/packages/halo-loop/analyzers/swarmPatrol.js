"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.swarmPatrolAnalyzer = swarmPatrolAnalyzer;
const dnsChecker_1 = require("./microAgents/dnsChecker");
const healthChecker_1 = require("./microAgents/healthChecker");
const envDriftChecker_1 = require("./microAgents/envDriftChecker");
const quotaChecker_1 = require("./microAgents/quotaChecker");
const buildChecker_1 = require("./microAgents/buildChecker");
const dependencyChecker_1 = require("./microAgents/dependencyChecker");
/**
 * Swarm Repair Patrol: Orchestrates 100+ micro-agents
 * Each agent does one simple check, collectively forming system health picture
 */
async function swarmPatrolAnalyzer() {
    const microAgents = [
        dnsChecker_1.dnsChecker,
        healthChecker_1.healthChecker,
        envDriftChecker_1.envDriftChecker,
        quotaChecker_1.quotaChecker,
        buildChecker_1.buildChecker,
        dependencyChecker_1.dependencyChecker,
        // Add more micro-agents here as needed
    ];
    const results = [];
    const issues = [];
    // Run all micro-agents in parallel (swarm behavior)
    const agentResults = await Promise.allSettled(microAgents.map(async (agent) => {
        try {
            return await agent.check();
        }
        catch (error) {
            return {
                agentId: agent.id,
                checkName: agent.name,
                status: "red",
                message: `Agent failed: ${error.message}`,
            };
        }
    }));
    // Aggregate results
    for (const result of agentResults) {
        if (result.status === "fulfilled") {
            results.push(result.value);
        }
        else {
            results.push({
                agentId: "unknown",
                checkName: "unknown",
                status: "red",
                message: `Agent execution failed: ${result.reason}`,
            });
        }
    }
    // Convert amber/red statuses to issues
    for (const result of results) {
        if (result.status === "amber" || result.status === "red") {
            issues.push({
                analyzer: "swarmPatrol",
                id: result.agentId,
                severity: result.status === "red" ? "critical" : "warning",
                description: `${result.checkName}: ${result.message}`,
                metadata: result.metadata,
            });
        }
    }
    // Calculate swarm health score
    const greenCount = results.filter((r) => r.status === "green").length;
    const amberCount = results.filter((r) => r.status === "amber").length;
    const redCount = results.filter((r) => r.status === "red").length;
    const totalCount = results.length;
    const healthScore = totalCount > 0 ? (greenCount / totalCount) * 100 : 0;
    return {
        analyzer: "swarmPatrol",
        issues,
        summary: `Swarm Patrol: ${greenCount} green, ${amberCount} amber, ${redCount} red (${healthScore.toFixed(1)}% healthy)`,
        metadata: {
            totalAgents: totalCount,
            greenCount,
            amberCount,
            redCount,
            healthScore,
            results,
        },
    };
}
