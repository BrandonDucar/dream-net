...
exports.syncIdentitiesFromContext = syncIdentitiesFromContext;
...
function syncIdentitiesFromContext(ctx) {
    const now = Date.now();
    ...
    if (ctx.dreamCortex?.listDreams) {
        ...
        dreams.forEach((d) => {
            identityStore_1.IdentityStore.upsertNode({
                id: `dream:${d.id}`,
                type: "dream",
                label: d.name,
                tags: d.tags ?? ["dream"],
    ...
    if (ctx.reputationLattice?.status && ctx.dreamCortex?.listDreams) {
        ...
        scores.forEach((s) => {
            ...
            identityStore_1.IdentityStore.addEdge(edge);
        });
    }
