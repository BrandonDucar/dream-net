...
exports.draftGenerator = exports.DraftGenerator = void 0;
const researchEngine_1 = require("./researchEngine");
const relevanceEngine_1 = require("./relevanceEngine");
const geoAwareness_1 = require("./geoAwareness");
const learningLoop_1 = require("./learningLoop");
class DraftGenerator {
    ...
    async generateDraft(recipientEmail, recipientName, recipientCompany, options) {
        // Layer 1: Research
        const research = await researchEngine_1.researchEngine.researchRecipient(recipientEmail, recipientName, recipientCompany);
        // Layer 2: Relevance
        const topics = await relevanceEngine_1.relevanceEngine.findRelevantTopics(recipientEmail, recipientCompany, recipientCompany);
        // Layer 3: Geo Awareness
        const geoContext = await geoAwareness_1.geoAwareness.getGeoContext(recipientEmail, recipientName, recipientCompany);
        // Generate subject line
        const subject = this.generateSubject(recipientName, recipientCompany, topics, research);
        // Generate body
        const body = this.generateBody(recipientName, recipientCompany, research, topics, geoContext, options);
     ...
     * Generate subject line
     */
    generateSubject(name, company, topics, research) {
        // Use learning loop for best performing patterns
        const bestSubjectPattern = learningLoop_1.learningLoop.getBestPattern('subject_line');
     ...
     * Generate email body
     */
    generateBody(name, company, research, topics, geoContext, options) {
     ...
     */
    generateVariants(subject, body, options) {
        const variants = [];
        // Subject variants
        variants.push({
            id: 'subject-1',
     ...
     */
    generateContentTwins(subject, body) {
        const twins = [];
        // Twitter version
        twins.push({
            platform: 'twitter',
...
exports.DraftGenerator = DraftGenerator;
exports.draftGenerator = new DraftGenerator();
