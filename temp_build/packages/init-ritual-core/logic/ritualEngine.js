...
exports.ensureDefaultTemplateSeeded = ensureDefaultTemplateSeeded;
exports.advanceIdentityInit = advanceIdentityInit;
exports.completeInitStepForIdentity = completeInitStepForIdentity;
const initStore_1 = require("../store/initStore");
let templateInitialized = false;
/**
 * Seed a default init flow template if none exist.
 */
function ensureDefaultTemplateSeeded() {
    if (templateInitialized)
        return;
    const existing = initStore_1.InitStore.listTemplates();
    ...
    initStore_1.InitStore.upsertTemplate(defaultTemplate);
    templateInitialized = true;
}
/**
 ...
 */
function advanceIdentityInit(ctx, identityId, templateId = "default-user-init") {
    ensureDefaultTemplateSeeded();
    const template = initStore_1.InitStore.getTemplate(templateId);
    if (!template) {
        throw new Error(`Init template not found: ${templateId}`);
    }
    const state = initStore_1.InitStore.getOrCreateIdentityState(identityId, templateId);
    ...
    if (!next) {
        ...
        initStore_1.InitStore.updateIdentityState(state);
        return { state, nextStep: undefined };
    }
    // Determine new stage
 ...
 */
function completeInitStepForIdentity(identityId, stepId) {
    const state = initStore_1.InitStore.getIdentityState(identityId);
    if (!state) {
    ...
    const template = initStore_1.InitStore.getTemplate(state.templateId);
    if (template) {
    ...
    return initStore_1.InitStore.updateIdentityState(state);
}
