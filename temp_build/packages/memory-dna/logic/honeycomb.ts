 ...
 */
export function getHexNeighbors(center: HexCoordinates): HexCoordinates[] {
 ...
 */
export function getSpiralCoordinate(n: number): HexCoordinates {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        for (let j = 0; j < radius; j++) {
            if (remainingSteps === 0) return current;
            current = {
                q: current.q + dir.q,
                r: current.r + dir.r,
                s: current.s + dir.s,
 ...
 */
export function areHexEqual(a: HexCoordinates, b: HexCoordinates): boolean {
