...
exports.registerShieldCoreSubscriber = registerShieldCoreSubscriber;
exports.registerJaggySubscriber = registerJaggySubscriber;
exports.registerDreamScopeSubscriber = registerDreamScopeSubscriber;
exports.getDreamScopeEvents = getDreamScopeEvents;
exports.getDreamScopeMetrics = getDreamScopeMetrics;
// Shield Core risk tracking (optional import)
let updateRiskProfile = null;
try {
 ...
 */
function registerShieldCoreSubscriber(bus) {
    // Subscribe to shield events
    bus.subscribe("SHIELD_EVENT", async (event) => {
        // TODO: integrate with real Shield Core logic
        console.info(`[shield-core] ${event.kind}`, {
            id: event.id,
            priority: event.priority,
            phase: event.payload.phase,
            threatType: event.payload.threatType,
            threatLevel: event.payload.threatLevel,
            actionTaken: event.payload.actionTaken,
            integrity: event.payload.integrity,
            traceId: event.context.traceId,
            sampled: event.context.sampled,
        });
    });
    // Subscribe to HTTP_REQUEST for threat heuristics
    bus.subscribe("HTTP_REQUEST", (event) => {
    ...
    bus.subscribeAll(async (event) => {
        ...
        if (!callerId || !ctx.tierId)
            return;
        const isFailure = event.kind === "THREAT_DETECTED" ||
            payload.ok === false ||
            payload.decision === "deny" ||
            payload.decision === "throttle" ||
            payload.reason === "TIMEOUT";
        const isHighRiskTool = typeof payload.toolId === "string" &&
            ["env.set", "env.delete", "api.rotateKey", "vercel.deploy"].includes(payload.toolId);
        ...
        const profile = updateRiskProfile({
            ...
            isFailure,
            isHighRiskTool,
            portId: payload.portId ?? undefined,
 ...
 * @param bus - Nerve bus instance
 */
function registerJaggySubscriber(bus) {
    const countsByKind = {};
    bus.subscribeAll((event) => {
        ...
        if (event.kind === "THREAT_DETECTED" ||
            ...
            (event.kind === "REQUEST_DECISION" && event.payload.decision === "deny")) {
 ...
 */
function registerDreamScopeSubscriber(bus) {
    ...
    bus.subscribe("SHIELD_EVENT", (event) => {
        // Add to ring buffer
        dreamScopeEvents.push(event);
        // Maintain ring buffer size
        if (dreamScopeEvents.length > DREAMSCOPE_RING_BUFFER_SIZE) {
 ...
 */
function getDreamScopeEvents(limit = 100) {
