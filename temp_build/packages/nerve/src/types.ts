 ...
 */
export type NerveEventKind =
  | "REQUEST_DECISION" // allow/deny/throttle
  | "SHIELD_PHASE_TICK" // shield cycles
  | "THREAT_DETECTED"
  | "THREAT_MITIGATED"
  | "SEO_APPLIED"
  | "GEOFENCE_APPLIED"
  | "DREAMSTATE_DECISION" // office/cabinet enforcement
  | "TREASURY_MOVE"
  | "BET_PLACED"
  | "ZEN_SESSION"
  | "METRIC_SNAPSHOT"
...
export interface ShieldEventPayload {
  phase: string; // alpha, beta, ... cellular
  threatType?: string;
  threatLevel?: string; // low → extreme
  actionTaken?: string; // block, honeypot, trace, etc.
 ...
 */
export type NerveEventPayload =
  | RequestDecisionPayload
  | ShieldEventPayload
  | AiSeoPayload
  | DreamStateDecisionPayload
  | TreasuryMovePayload
  | BetPlacedPayload
  | ZenSessionPayload
