...
exports.buildSynapses = buildSynapses;
const index_ts_1 = require("../event-wormholes/src/index.ts");
let synapseMap = {};
let synapseConnections = [];
/**
 ...
 */
function buildSynapses(systems) {
    synapseMap = {
        swarm: systems.swarm,
 ...
 */
buildSynapses.pulse = async (from, to, event) => {
    const connection = synapseConnections.find((c) => c.from === from && c.to === to && c.enabled);
    if (!connection) {
