...
exports.withPort = withPort;
const ports_1 = require("./ports");
const rateLimiter_1 = require("../../dreamnet-control-core/rateLimiter");
// Shield Core risk tracking (optional import)
let getRiskProfile = null;
try {
...
function withPort(portId) {
    return (req, res, next) => {
        const traceId = req.traceId || "unknown";
        const callerIdentity = req.callerIdentity;
        const portProfile = (0, ports_1.getPortProfile)(portId);
        if (!portProfile) {
        ...
        if (!isGodVault && portProfile.requiredOfficeIds && portProfile.requiredOfficeIds.length > 0) {
            const hasRequiredOffice = portProfile.requiredOfficeIds.some((officeId) => hasOffice(callerIdentity, officeId));
        ...
        if (!isGodVault && portProfile.requiredCabinetIds && portProfile.requiredCabinetIds.length > 0) {
            const hasRequiredCabinet = portProfile.requiredCabinetIds.some((cabinetId) => hasCabinet(callerIdentity, cabinetId));
        ...
        if (!isGodVault && getRiskProfile) {
            const callerId = callerIdentity.passport?.citizenId ||
        ...
        if (!isGodVault) {
            ...
            const rateCheck = (0, rateLimiter_1.checkAndConsume)(portProfile.clusterId || portId, callerIdentity.tierId, effectiveLimit);
