 ...
 */
function buildFeed(ctx, query) {
    const allPosts = socialStore_1.SocialStore.listPosts();
    ...
    const items = filtered.map((post) => {
        const comments = socialStore_1.SocialStore.listCommentsForPost(post.id);
        const reactions = socialStore_1.SocialStore.listReactionsForPost(post.id);
