...
function computeScore(
  ...
  if (ctx.reputationLattice?.status) {
    ...
    const rep = scores.find(
      (s: any) =>
        `${s.entityType}:${s.entityId}` === post.authorIdentityId
    );
...
export function buildFeed(
  ...
  if (query.forIdentityId) {
    ...
    filtered = filtered.filter(
      (p) =>
        p.authorIdentityId === query.forIdentityId ||
        (p.refs ?? []).some(
          (r) =>
            r.type === "identity" && r.id === query.forIdentityId
        )
    );
  }
  ...
  if (query.forDreamId) {
    filtered = filtered.filter((p) =>
      (p.refs ?? []).some(
        (r) => r.type === "dream" && r.id === query.forDreamId
      )
    );
  }

  const items: FeedItem[] = filtered.map((post) => {
