...
exports.runFunnelWebSpider = runFunnelWebSpider;
exports.ensureDefaultSensors = ensureDefaultSensors;
...
async function checkSensor(sensor, ctx) {
    ...
    switch (sensor.type) {
        case "twilio":
            ...
            if (sensor.config.accountSid && sensor.config.authToken) {
                try {
                    const apiFlies = await (0, apiIntegrations_1.checkTwilioAPI)(sensor.config.accountSid, sensor.config.authToken, sensor.config.phoneNumber);
                    flies.push(...apiFlies);
                }
                catch (err) {
            ...
            if (sensor.config.simulateFlies && flies.length === 0) {
                const fly = (0, flyCatcher_1.createFly)("message", "twilio", { message: "Simulated Twilio SMS", from: "+1234567890" }, "medium", true);
                flies.push(fly);
            }
            break;
        case "telegram":
            ...
            if (sensor.config.botToken) {
                try {
                    const apiFlies = await (0, apiIntegrations_1.checkTelegramAPI)(sensor.config.botToken, sensor.config.chatId);
                    flies.push(...apiFlies);
                }
                catch (err) {
            ...
            if (sensor.config.simulateFlies && flies.length === 0) {
                const fly = (0, flyCatcher_1.createFly)("message", "telegram", { message: "Simulated Telegram message", chatId: "12345" }, "medium", true);
                flies.push(fly);
            }
            break;
        case "twitter":
            ...
            if (sensor.config.bearerToken) {
                try {
                    const apiFlies = await (0, apiIntegrations_1.checkTwitterAPI)(sensor.config.bearerToken, sensor.config.username);
                    flies.push(...apiFlies);
                }
                catch (err) {
            ...
            // Fallback to simulation
            if (sensor.config.simulateFlies && flies.length === 0) {
                const fly = (0, flyCatcher_1.createFly)("mention", "twitter", { tweet: "Simulated Twitter mention", username: "@dreamnet" }, "low", false);
                flies.push(fly);
            }
            break;
        case "webhook":
            ...
            if (sensor.config.simulateFlies) {
                const fly = (0, flyCatcher_1.createFly)("webhook", "webhook", { type: "funding", data: { leadId: "lead:test" } }, "high", true);
                flies.push(fly);
            }
            break;
        case "blockchain":
            ...
            if (sensor.config.simulateFlies) {
                const fly = (0, flyCatcher_1.createFly)("transaction", "blockchain", { txHash: "0x123...", value: "1000000000000000000" }, "high", true);
                flies.push(fly);
            }
            break;
        case "email":
            ...
            if (sensor.config.simulateFlies) {
                const fly = (0, flyCatcher_1.createFly)("message", "email", { subject: "Simulated email", from: "test@example.com" }, "medium", true);
                flies.push(fly);
            }
            break;
        default:
    ...
    if (flies.length > 0) {
        ...
        spiderStore_1.SpiderStore.updateSensorCatchRate(sensor.id, sensor.catchRate);
    }
    return flies;
}
/**
 ...
 */
async function runFunnelWebSpider(ctx) {
    const newThreads = [];
    const activeSensors = spiderStore_1.SpiderStore.listActiveSensors();
    ...
    for (const sensor of activeSensors) {
        try {
            ...
            for (const fly of flies) {
                const thread = (0, flyCatcher_1.catchFly)(fly);
                if (thread) {
                    spiderStore_1.SpiderStore.addThread(thread);
...
function ensureDefaultSensors() {
    const existing = spiderStore_1.SpiderStore.listSensors();
    ...
    for (const sensor of sensors) {
        spiderStore_1.SpiderStore.addSensor(sensor);
    }
    // Log sensor status
    if (twilioActive) {
...
function updateSensorsFromEnv() {
    const sensors = spiderStore_1.SpiderStore.listSensors();
    for (const sensor of sensors) {
        let updated = false;
        ...
        if (updated) {
            spiderStore_1.SpiderStore.updateSensor(sensor);
        }
    }
}
