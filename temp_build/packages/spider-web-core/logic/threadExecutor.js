...
exports.executeThread = executeThread;
exports.canExecuteThread = canExecuteThread;
const spiderStore_1 = require("../store/spiderStore");
/**
 * Execute a thread's execution plan
 */
async function executeThread(thread, ctx) {
 ...
 */
function canExecuteThread(thread) {
    if (!thread.dependsOn || thread.dependsOn.length === 0) {
        return true;
    }
    // Check if all dependencies are completed
    for (const depId of thread.dependsOn) {
        const depThread = spiderStore_1.SpiderStore.getThread(depId);
