...
exports.applyTemplateToThread = applyTemplateToThread;
const spiderStore_1 = require("../store/spiderStore");
/**
 * Ensure default thread templates exist
 */
function ensureDefaultTemplates() {
    const existing = spiderStore_1.SpiderStore.listTemplates();
    ...
    for (const template of templates) {
        spiderStore_1.SpiderStore.addTemplate(template);
    }
    return templates;
}
/**
 ...
 */
function findMatchingTemplate(thread) {
    const templates = spiderStore_1.SpiderStore.listTemplates();
    // Find template that matches thread kind and source/target patterns
    return templates.find((t) => {
        if (t.kind !== thread.kind)
            return false;
        // Check source pattern
        if (t.sourcePattern.kind !== thread.source.kind)
            return false;
        // Check target patterns (at least one match)
        const targetMatch = t.targetPattern.some((tp) => thread.targets.some((tt) => tp.kind === tt.kind));
        return targetMatch;
    });
}
/**
 ...
 */
function applyTemplateToThread(thread, template) {
