...
exports.performNeuralHandshake = performNeuralHandshake;
exports.releaseBeneficialSwarm = releaseBeneficialSwarm;
 ...
 */
async function performNeuralHandshake(sporeA, sporeB) {
    // 1. Proximity Check
    ...
    if (knowledgeGap > 5) {
        ...
        // Log the "Infection"
        await VectorStore_1.vectorStore.addMemory(`VIRAL TRANSMISSION: Spore ${sporeA.entityId} infected ${sporeB.entityId} with updated knowledge.`, { type: "viral_infection", source: sporeA.entityId, target: sporeB.entityId });
    }
    return {
        success: true,
 ...
 */
async function releaseBeneficialSwarm(seedSporeId, swarmType) {
    await VectorStore_1.vectorStore.addMemory(`SWARM RELEASE: ${swarmType} initiated from Patient Zero ${seedSporeId}`, { type: "swarm_start", seed: seedSporeId, swarmType });
