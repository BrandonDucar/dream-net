...
exports.ResonanceOptimizer = void 0;
...
class ResonanceOptimizer {
     ...
     */
    static async getResonance(chainId) {
        ...
        const memories = await VectorStore_1.vectorStore.query(`${chainId} crypto trend sentiment`, 50);
        let socialVolume = 0;
        let positiveSentiment = 0;
        if (memories.length > 0) {
            // Filter: Ensure memory is actually about this chain (handling Mock Embedding noise)
            const relevantMemories = memories.filter(m => m.metadata?.chain?.toLowerCase() === chainId.toLowerCase() ||
                m.text.toLowerCase().includes(chainId.toLowerCase()));
            if (relevantMemories.length > 0) {
                ...
                const positiveCount = relevantMemories.filter(m => {
                    ...
                    if (hit)
                        console.log(`   ✨ Sentiment Hit: "${m.text.substring(0, 40)}..."`);
                    return hit;
                }).length;
...
exports.ResonanceOptimizer = ResonanceOptimizer;
