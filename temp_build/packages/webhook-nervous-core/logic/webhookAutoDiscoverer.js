...
exports.autoDiscoverWebhooksFromEnv = autoDiscoverWebhooksFromEnv;
exports.autoDiscoverWebhooksFromConfig = autoDiscoverWebhooksFromConfig;
exports.autoDiscoverAllWebhooks = autoDiscoverAllWebhooks;
exports.autoCreateDefaultAntibodies = autoCreateDefaultAntibodies;
const nervousSystem_1 = require("./nervousSystem");
const immuneSystem_1 = require("./immuneSystem");
/**
 * Auto-discover webhooks from environment variables
 */
function autoDiscoverWebhooksFromEnv() {
    const discovered = [];
    ...
    for (const [key, value] of Object.entries(process.env)) {
        ...
        if (isWebhook) {
            try {
                ...
                const neuron = (0, nervousSystem_1.createNeuron)(name, value, type, {
                    threshold: 0.5,
                    adaptation: 0.1,
 ...
 */
function autoDiscoverWebhooksFromConfig() {
    const discovered = [];
    ...
    const scanObject = (obj, prefix = "") => {
        for (const [key, value] of Object.entries(obj)) {
            ...
            if (typeof value === "string" && value.startsWith("http")) {
                ...
                const neuron = (0, nervousSystem_1.createNeuron)(name, value, "motor", {
                    threshold: 0.5,
                    adaptation: 0.1,
                });
                discovered.push(neuron);
 ...
 */
function autoDiscoverAllWebhooks() {
    const discovered = [];
 ...
 */
function autoCreateDefaultAntibodies() {
    // SQL Injection protection
    (0, immuneSystem_1.createAntibody)("SQL Injection Detector", "(?i)(union|select|insert|delete|drop|exec|script)", "block", { memory: true, effectiveness: 0.95 });
    // XSS protection
    (0, immuneSystem_1.createAntibody)("XSS Detector", "(?i)(<script|javascript:|onerror=|onload=)", "block", { memory: true, effectiveness: 0.9 });
    // Rate limit detection
    (0, immuneSystem_1.createAntibody)("Rate Limit Detector", "(?i)(rate.?limit|too.?many|429)", "alert", { memory: true, effectiveness: 0.8 });
    // Timeout detection
    (0, immuneSystem_1.createAntibody)("Timeout Detector", "(?i)(timeout|timed.?out|504|408)", "alert", { memory: true, effectiveness: 0.85 });
