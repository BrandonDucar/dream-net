...
exports.generateWhaleOutreachDraft = generateWhaleOutreachDraft;
exports.createWhaleGmailDraft = createWhaleGmailDraft;
const inbox_squared_core_1 = require("@dreamnet/inbox-squared-core");
/**
 * Generate outreach email for Whale Pack partner/influencer
 */
async function generateWhaleOutreachDraft(target, product, audience, options) {
    if (!target.email) {
        return null;
    }
    const draftOptions = {
    ...
    try {
        const draft = await inbox_squared_core_1.inboxSquared.generateDraft(target.email, target.name, target.company || target.role, draftOptions);
 ...
 */
async function createWhaleGmailDraft(draft) {
    try {
