...
exports.ensureSeedProductsAndAudiences = ensureSeedProductsAndAudiences;
exports.generateNewContentPlans = generateNewContentPlans;
...
function ensureSeedProductsAndAudiences() {
    // Only seed if empty
    if (whaleStore_1.WhaleStore.listProducts().length === 0) {
        const now = Date.now();
        whaleStore_1.WhaleStore.upsertProduct({
    ...
    if (whaleStore_1.WhaleStore.listAudiences().length === 0) {
        const now = Date.now();
        whaleStore_1.WhaleStore.upsertAudience({
            id: "aud:us-fl-dads",
...
function generateNewContentPlans(ctx, desiredCount = 5) {
    const products = whaleStore_1.WhaleStore.listProducts().filter((p) => p.active);
    const audiences = whaleStore_1.WhaleStore.listAudiences();
    const existingPlans = whaleStore_1.WhaleStore.listPlans();
    ...
    for (let i = 0; i < ideasToGenerate; i++) {
        ...
        whaleStore_1.WhaleStore.upsertPlan(plan);
        newPlans.push(plan);
...
function pickHookStyleFor(product, audience) {
    const tags = new Set([...(product.tags ?? []), ...(audience.tags ?? [])]);
    if (tags.has("tactical") || tags.has("military"))
        return "storytime";
    if (tags.has("travel"))
        return "tutorial";
    if (tags.has("ai"))
...
function buildHookLine(product, audience, style) {
    switch (style) {
...
function buildScript(product, audience, style) {
    return [
...
function buildHashtags(product, audience) {
    ...
    if (audience.geo)
        tags.add(audience.geo.toLowerCase());
    return Array.from(tags).slice(0, 8);
}
function pickSoundHint(product, audience) {
