...
exports.analyzeEmailEffectiveness = analyzeEmailEffectiveness;
const analystStore_1 = require("../store/analystStore");
/**
 * Generate insights from current data and learned patterns
 */
function generateInsights(leads, queueItems, patterns) {
    const insights = [];
    const now = Date.now();
    // Insight 1: High-value leads not yet contacted
    const highValueNew = leads.filter((l) => l.stage === "new" &&
    ...
    const oldLeads = leads.filter((l) => l.stage === "contacted" && Date.now() - l.updatedAt > 7 * 24 * 60 * 60 * 1000);
...
function generatePredictions(leads, patterns) {
    ...
    for (const lead of candidates) {
        ...
        const vcPattern = patterns.find((p) => p.type === "stage_progression" && p.pattern.includes("VC"));
    ...
    // Store predictions
    predictions.forEach((p) => analystStore_1.AnalystStore.upsertPrediction(p));
    return predictions;
}
/**
 * Analyze email effectiveness
 */
function analyzeEmailEffectiveness(queueItems, leads) {
    const analyses = [];
    const now = Date.now();
    for (const item of queueItems) {
        ...
        const predictedResponseRate = subjectLineScore * 0.3 +
            bodyLengthScore * 0.2 +
    ...
    // Store analyses
    analyses.forEach((a) => analystStore_1.AnalystStore.upsertEmailEffectiveness(a));
    return analyses;
}
