...
exports.generateFollowUpDraftForLead = generateFollowUpDraftForLead;
 ...
 */
function generateFollowUpDraftForLead(lead, opts) {
    if (!lead.email) {
        return null;
    }
    // Find previous email draft to reference
    const previousDrafts = fundingStore_1.FundingStore.listDraftsForLead(lead.id);
