...
exports.computeRewardsForSession = computeRewardsForSession;
...
function computeRewardsForSession(ctx, session) {
    const activities = gardenStore_1.GardenStore.listActivitiesForSession(session.id);
    if (!activities.length)
        return [];
    const now = Date.now();
    const totalMinutes = activities.reduce((sum, a) => sum + (a.durationMinutes ?? 0), 0);
    const avgIntensity = activities.reduce((sum, a) => sum + (a.intensity ?? 0.5), 0) /
        activities.length;
    ...
    rewards.push({
        ...
        currency: "ZEN_POINTS",
        reason: `Zen session with ${totalMinutes} minutes and avg intensity ${avgIntensity.toFixed(2)}.`,
        createdAt: now,
    });
    ...
    // Persist them
    rewards.forEach((r) => gardenStore_1.GardenStore.addReward(r));
    return rewards;
}
