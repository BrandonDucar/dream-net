...
import { createOctopusTriggerRouter } from "./routes/octopus-trigger";
...
import { createLiquidityRouter } from "./routes/liquidity";
import { createMechanicRouter } from "./routes/mechanic";
import { createVibesRouter } from "./routes/vibes";
import { createCreativeRouter } from "./routes/creative";
import { createHunterRouter } from "./routes/hunter";
import { Telepathy } from './mesh/telepathy';
import { QuantumMechanic } from './agents/quantum-mechanic';
import { VibeConductor } from './agents/vibe-conductor';
...
app.use("/api", createOctopusTriggerRouter()); // Added for GCP Workflows
...
app.use("/api", createLiquidityRouter());
app.use("/api/mechanic", createMechanicRouter());
app.use("/api/vibes", createVibesRouter());
app.use("/api/creative", createCreativeRouter());
app.use("/api/hunter", createHunterRouter());
...
// The imports above have already instantiated the agents (Singleton pattern)
console.log("⚡ [Nervous System] Awakening...");
console.log(`   🧠 Universal Nervous System: ${Telepathy ? 'Online' : 'Offline'}`);
console.log(`   ⚕️  Quantum Mechanic: Active`);
console.log(`   🎵 Vibe Conductor: Listening`);
...
async function initOptionalSubsystems(app: Express, server: Server): Promise<void> {
  ...
  if (shouldInitHeavy) {
    ...
    // Initialize Liquidity Engine - Tier IV Subsystem (SLU Tokenomics)
    try {
      // Import and initialize configs (including SLU pools)
      LiquidityEngine = (await import("../packages/liquidity-engine")).LiquidityEngine;
      LiquidityEngine.initConfigs();

      const liqStatus = LiquidityEngine.status();
      console.log(`💧 [Liquidity Engine] Initialized - ${liqStatus.poolCount} pools planned (${liqStatus.deployedCount} deployed)`);
      console.log(`   🪙 SLU Model: stSPK staking pools configured`);
    } catch (error) {
  ...
  if (shouldInitHeavy) {
    ...
    try {
      ...
      const osStatus = DreamNetOSCore.run({
        ...
        dreamStateCore: DreamStateCoreInstance,
        spiderWebCore: SpiderWebCore,
        neuralMesh: { remember: () => console.log("Stubbed NeuralMesh") }, // Inline stub to force boot
      ...
      // Initialize Neural Mesh - Agent Communication Layer 🧠
      try {
        // const neuralMeshModule = await import("@dreamnet/neural-mesh");
        // NeuralMesh = neuralMeshModule.NeuralMesh || { remember: () => console.log("Stubbed NeuralMesh remember") };
        NeuralMesh = { remember: () => console.log("Stubbed NeuralMesh remember") };
        console.log(`🧠 [Neural Mesh] Stubbed - Advanced agent communication bypass`);
      } catch (error: any) {
        console.warn("[Neural Mesh] Initialization warning:", error.message);
        // Fallback to stub to prevent crash
        NeuralMesh = { remember: () => { } };
      }

      // 12. Antigravity & Guardian (The Ghost & The Shield) - Mission 10 & 11
      try {
        const { Antigravity } = await import("@dreamnet/antigravity-core");
        const { GuardianFramework } = await import("@dreamnet/guardian-framework-core");

        console.log("👻 [System] Initializing Antigravity Core...");
        Antigravity.reflect(); // The Architect wakes up

        console.log("🛡️ [System] Activating Guardian Framework...");
        GuardianFramework.runCycle();
      } catch (e: any) {
        console.warn("⚠️ [Antigravity/Guardian] Initialization failed (Optional):", e.message);
      }
