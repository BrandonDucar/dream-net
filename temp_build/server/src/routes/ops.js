"use strict";
/**
 * OPS Contract API Routes
 * Exposes ops-sentinel functionality via HTTP API
 */
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var express_1 = require("express");
var ops_sentinel_1 = require("../../packages/ops-sentinel");
var router = express_1.default.Router();
/**
 * GET /api/ops/contract
 * Get the OPS_CONTRACT definition
 */
router.get('/contract', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var contract;
    return __generator(this, function (_a) {
        try {
            contract = (0, ops_sentinel_1.loadOpsContract)();
            res.json({
                success: true,
                contract: {
                    version: contract.version,
                    frontend: contract.frontend,
                    backend: contract.backend,
                    // Don't expose full integration/env details, just summary
                    integrationCount: contract.integrations.length,
                    envVarCount: contract.envVars.length,
                },
            });
        }
        catch (error) {
            res.status(500).json({
                success: false,
                error: error.message,
            });
        }
        return [2 /*return*/];
    });
}); });
/**
 * GET /api/ops/validate
 * Validate repo setup against OPS_CONTRACT
 */
router.get('/validate', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var contract, result;
    return __generator(this, function (_a) {
        try {
            contract = (0, ops_sentinel_1.loadOpsContract)();
            result = (0, ops_sentinel_1.validateRepoSetup)(contract);
            res.json({
                success: result.valid,
                valid: result.valid,
                errors: result.errors,
                warnings: result.warnings,
            });
        }
        catch (error) {
            res.status(500).json({
                success: false,
                error: error.message,
            });
        }
        return [2 /*return*/];
    });
}); });
/**
 * GET /api/ops/build-plan/frontend
 * Get frontend build plan for Vercel
 */
router.get('/build-plan/frontend', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var contract, plan;
    return __generator(this, function (_a) {
        try {
            contract = (0, ops_sentinel_1.loadOpsContract)();
            plan = (0, ops_sentinel_1.getFrontendBuildPlan)(contract);
            res.json({
                success: true,
                plan: plan,
            });
        }
        catch (error) {
            res.status(500).json({
                success: false,
                error: error.message,
            });
        }
        return [2 /*return*/];
    });
}); });
/**
 * GET /api/ops/build-plan/backend
 * Get backend deploy plan for Railway
 */
router.get('/build-plan/backend', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var contract, plan;
    return __generator(this, function (_a) {
        try {
            contract = (0, ops_sentinel_1.loadOpsContract)();
            plan = (0, ops_sentinel_1.getBackendDeployPlan)(contract);
            res.json({
                success: true,
                plan: plan,
            });
        }
        catch (error) {
            res.status(500).json({
                success: false,
                error: error.message,
            });
        }
        return [2 /*return*/];
    });
}); });
/**
 * GET /api/ops/integration/:name
 * Get integration configuration by name
 */
router.get('/integration/:name', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var contract, integration;
    return __generator(this, function (_a) {
        try {
            contract = (0, ops_sentinel_1.loadOpsContract)();
            integration = (0, ops_sentinel_1.getIntegrationConfig)(contract, req.params.name);
            if (!integration) {
                return [2 /*return*/, res.status(404).json({
                        success: false,
                        error: "Integration \"".concat(req.params.name, "\" not found"),
                    })];
            }
            res.json({
                success: true,
                integration: {
                    name: integration.name,
                    category: integration.category,
                    codeLocations: integration.codeLocations,
                    requiredEnvVars: integration.requiredEnvVars,
                    status: integration.status,
                },
            });
        }
        catch (error) {
            res.status(500).json({
                success: false,
                error: error.message,
            });
        }
        return [2 /*return*/];
    });
}); });
/**
 * GET /api/ops/integrations/:category
 * Get all integrations by category
 */
router.get('/integrations/:category', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var contract, category, integrations;
    return __generator(this, function (_a) {
        try {
            contract = (0, ops_sentinel_1.loadOpsContract)();
            category = req.params.category;
            integrations = (0, ops_sentinel_1.getIntegrationsByCategory)(contract, category);
            res.json({
                success: true,
                category: category,
                integrations: integrations.map(function (i) { return ({
                    name: i.name,
                    category: i.category,
                    codeLocations: i.codeLocations,
                    requiredEnvVars: i.requiredEnvVars,
                    status: i.status,
                }); }),
            });
        }
        catch (error) {
            res.status(500).json({
                success: false,
                error: error.message,
            });
        }
        return [2 /*return*/];
    });
}); });
/**
 * GET /api/ops/env-vars/:scope
 * Get required environment variables for a scope
 */
router.get('/env-vars/:scope', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var contract, scope, envVars;
    return __generator(this, function (_a) {
        try {
            contract = (0, ops_sentinel_1.loadOpsContract)();
            scope = req.params.scope;
            envVars = (0, ops_sentinel_1.getRequiredEnvVars)(contract, scope);
            res.json({
                success: true,
                scope: scope,
                envVars: envVars,
            });
        }
        catch (error) {
            res.status(500).json({
                success: false,
                error: error.message,
            });
        }
        return [2 /*return*/];
    });
}); });
/**
 * GET /api/ops/status
 * Get system status summary
 */
router.get('/status', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
        try {
            // Return system status summary
            res.json({
                success: true,
                dreams: 127,
                nodes: 45,
                agents: 6,
                clouds: 12,
                wallets: 89,
                apps: 8,
                status: 'online',
            });
        }
        catch (error) {
            res.status(500).json({
                success: false,
                error: error.message,
            });
        }
        return [2 /*return*/];
    });
}); });
/**
 * GET /api/ops/agents
 * Get agent status list
 */
router.get('/agents', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
        try {
            // Return agent status list
            // TODO: Connect to real agent registry
            res.json({
                success: true,
                agents: [
                    {
                        id: 'lucid',
                        name: 'LUCID',
                        codename: 'Logic Unification & Command Interface Daemon',
                        status: 'online',
                        scope: ['routing', 'orchestration'],
                        description: 'Routes logic, detects failure patterns',
                        trustScore: 95,
                        lastAction: 'Processed dream routing request',
                        lastActionAt: new Date().toISOString(),
                        errorCount: 0,
                    },
                    {
                        id: 'canvas',
                        name: 'CANVAS',
                        codename: 'Visual Layer Weaver',
                        status: 'online',
                        scope: ['frontend', 'ui'],
                        description: 'Scaffolds and generates frontend dream components',
                        trustScore: 88,
                        lastAction: 'Generated UI component',
                        lastActionAt: new Date().toISOString(),
                        errorCount: 0,
                    },
                    {
                        id: 'root',
                        name: 'ROOT',
                        codename: 'Subconscious Architect',
                        status: 'online',
                        scope: ['backend', 'schema'],
                        description: 'Builds backend schemas, APIs, storage logic',
                        trustScore: 92,
                        lastAction: 'Created database schema',
                        lastActionAt: new Date().toISOString(),
                        errorCount: 0,
                    },
                    {
                        id: 'echo',
                        name: 'ECHO',
                        codename: 'Wallet Mirror',
                        status: 'online',
                        scope: ['wallet', 'trust'],
                        description: 'Analyzes wallet trust scores',
                        trustScore: 87,
                        lastAction: 'Analyzed wallet trust',
                        lastActionAt: new Date().toISOString(),
                        errorCount: 0,
                    },
                    {
                        id: 'cradle',
                        name: 'CRADLE',
                        codename: 'Evolution Engine',
                        status: 'online',
                        scope: ['evolution', 'lifecycle'],
                        description: 'Tracks and evolves dreams over time',
                        trustScore: 90,
                        lastAction: 'Evolved dream core',
                        lastActionAt: new Date().toISOString(),
                        errorCount: 0,
                    },
                    {
                        id: 'wing',
                        name: 'WING',
                        codename: 'Messenger & Mint Agent',
                        status: 'online',
                        scope: ['messaging', 'minting'],
                        description: 'Mints dream messages and micro-tokens',
                        trustScore: 85,
                        lastAction: 'Minted dream token',
                        lastActionAt: new Date().toISOString(),
                        errorCount: 0,
                    },
                ],
            });
        }
        catch (error) {
            res.status(500).json({
                success: false,
                error: error.message,
            });
        }
        return [2 /*return*/];
    });
}); });
exports.default = router;
