...
exports.setupSystemMappingRoutes = setupSystemMappingRoutes;
function setupSystemMappingRoutes(app) {
    // Get complete system topology
    app.get('/api/system-map/full-topology', function (req, res) {
        try {
            var topology = {
                nodes: [
    ...
    app.get('/api/system-scan/deep-analysis', function (req, res) {
        try {
            var deepAnalysis = {
                scanId: "scan-".concat(Date.now()),
                timestamp: new Date().toISOString(),
                levels: [
                    {
                        level: 0,
    ...
    app.get('/api/system-map/realtime-metrics', function (req, res) {
        try {
            var metrics = {
                timestamp: new Date().toISOString(),
                systemWide: {
